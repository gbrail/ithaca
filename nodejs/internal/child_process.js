...
var inspect = require('internal/util/inspect').inspect;
var assert = require('internal/assert');
var {
  Process
} = internalBinding('process_wrap');
var {
  WriteWrap,
  kReadBytesOrError,
  kArrayBufferOffset,
...
var spawn_sync = internalBinding('spawn_sync');
var {
  kStateSymbol
} = require('internal/dgram');
var dc = require('diagnostics_channel');
var childProcessChannel = dc.channel('child_process');
var childProcessSpawn = dc.tracingChannel('child_process.spawn');
var {
  UV_EACCES,
  UV_EAGAIN,
  UV_EINVAL,
...
var freeParser;
var HTTPParser;
var MAX_HANDLE_RETRANSMISSIONS = 3;
var kChannelHandle = _Symbol('kChannelHandle');
var kIsUsedAsStdio = _Symbol('kIsUsedAsStdio');
var kPendingMessages = _Symbol('kPendingMessages');
  ...
  'net.Socket': {
    send(message, socket, options) {
      ...
      if (socket.server) {
        ...
        var firstTime = !this[kChannelHandle].sockets.send[message.key];
...
function ChildProcess() {
  ...
  this._handle.onexit = (exitCode, signalCode) => {
    ...
    if (exitCode < 0) {
      ...
      var err = new ErrnoException(exitCode, syscall);
...
ChildProcess.prototype.spawn = function spawn(options) {
  ...
  if (ipc !== undefined) {
    ...
    if (options.envPairs === undefined) options.envPairs = [];else validateArray(options.envPairs, 'options.envPairs');
  ...
  if (childProcessSpawn.hasSubscribers) {
    childProcessSpawn.start.publish({
      process: this,
      options
    });
  }
  ...
  for (i = 0; i < stdio.length; i++) {
    ...
    if (_stream.type === 'wrap') {
      _stream.handle.reading = false;
      _stream.handle.readStop();
      _stream._stdio.pause();
      _stream._stdio.readableFlowing = false;
      _stream._stdio._readableState.reading = false;
      _stream._stdio[kIsUsedAsStdio] = true;
      continue;
    }
    if (_stream.handle) {
  ...
  this.stdio = [];
  for (i = 0; i < stdio.length; i++) ArrayPrototypePush(this.stdio, stdio[i].socket === undefined ? null : stdio[i].socket);
...
var _refExplicitlySet = /*#__PURE__*/new WeakMap();
var Control = /*#__PURE__*/function (_EventEmitter) {
  function Control(channel) {
    var _this;
    _classCallCheck(this, Control);
    _this = _callSuper(this, Control);
    _classPrivateFieldInitSpec(_this, _channel, null);
    _classPrivateFieldInitSpec(_this, _refs, 0);
    _classPrivateFieldInitSpec(_this, _refExplicitlySet, false);
    _classPrivateFieldSet(_channel, _this, channel);
    _this[kPendingMessages] = [];
    return _this;
  }
  ...
  return _createClass(Control, [{
    key: "refCounted",
    value: function refCounted() {
      var _this$refs;
      if (_classPrivateFieldSet(_refs, this, (_this$refs = _classPrivateFieldGet(_refs, this), ++_this$refs)) === 1 && !_classPrivateFieldGet(_refExplicitlySet, this)) {
        _classPrivateFieldGet(_channel, this).ref();
      }
    }
  }, {
    key: "unrefCounted",
    value: function unrefCounted() {
      var _this$refs2;
      if (_classPrivateFieldSet(_refs, this, (_this$refs2 = _classPrivateFieldGet(_refs, this), --_this$refs2)) === 0 && !_classPrivateFieldGet(_refExplicitlySet, this)) {
        _classPrivateFieldGet(_channel, this).unref();
      }
    }
  }, {
    key: "ref",
    value: function ref() {
      _classPrivateFieldSet(_refExplicitlySet, this, true);
      _classPrivateFieldGet(_channel, this).ref();
    }
  }, {
    key: "unref",
    ...
    key: "fd",
    get: function () {
      return _classPrivateFieldGet(_channel, this) ? _classPrivateFieldGet(_channel, this).fd : undefined;
    }
  }]);
}(EventEmitter);
var serialization;
function setupChannel(target, channel, serializationMode) {
  ...
  target._pendingMessage = null;
  if (serialization === undefined) serialization = require('internal/child_process/serialization');
  var {
    initMessageChannel,
  ...
  channel.onread = function (arrayBuffer) {
    var recvHandle = channel.pendingHandle;
    channel.pendingHandle = null;
    if (arrayBuffer) {
      var nread = streamBaseState[kReadBytesOrError];
      var offset = streamBaseState[kArrayBufferOffset];
      var pool = new Uint8Array(arrayBuffer, offset, nread);
      if (recvHandle) pendingHandle = recvHandle;
      for (var message of parseChannelMessages(channel, pool)) {
  ...
  target.on('newListener', function () {
    process.nextTick(() => {
      ...
      var messages = target.channel[kPendingMessages];
      var {
        length
      } = messages;
      if (!length) return;
  ...
  target.send = function (message, handle, options, callback) {
    ...
    var ex = new ERR_IPC_CHANNEL_CLOSED();
  ...
  target._send = function (message, handle, options, callback) {
    ...
    var err = writeChannelMessage(channel, req, message, handle);
    var wasAsyncWrite = streamBaseState[kLastWriteWasAsync];
    if (err === 0) {
      if (handle) {
        this._handleQueue ||= [];
        if (obj?.postSend) obj.postSend(message, handle, options, callback, target);
      }
      if (wasAsyncWrite) {
        req.oncomplete = () => {
      ...
      if (obj?.postSend) obj.postSend(message, handle, options, callback);
      if (!options.swallowErrors) {
        var ex = new ErrnoException(err, 'write');
        if (typeof callback === 'function') {
          process.nextTick(callback, ex);
        } else {
          process.nextTick(() => this.emit('error', ex));
  ...
  function emit(event, message, handle) {
    ...
    ArrayPrototypePush(target.channel[kPendingMessages], [event, message, handle]);
  }
  ...
  stdio = ArrayPrototypeReduce(stdio, (acc, stdio, i) => {
    function cleanup() {
      for (var _i = 0; _i < acc.length; _i++) {
        if ((acc[_i].type === 'pipe' || acc[_i].type === 'ipc') && acc[_i].handle) acc[_i].handle.close();
      }
    }
    ...
    if (stdio === 'ignore') {
      ...
      if (sync || ipc !== undefined) {
        ...
        cleanup();
        if (!sync) throw new ERR_IPC_ONE_PIPE();else throw new ERR_IPC_SYNC_FORK();
      }
...
function getSocketList(type, worker, key) {
  var sockets = worker[kChannelHandle].sockets[type];
