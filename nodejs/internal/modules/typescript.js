 ...
 */
var loadTypeScriptParser = getLazy(() => {
  assertTypeScript();
  var amaro = require('internal/deps/amaro/dist/index');
...
function parseTypeScript(source, options) {
  var parse = loadTypeScriptParser();
  try {
    ...
    switch (error?.code) {
      case 'UnsupportedSyntax':
        {
          var unsupportedSyntaxError = new ERR_UNSUPPORTED_TYPESCRIPT_SYNTAX(error.message);
          throw decorateErrorWithSnippet(unsupportedSyntaxError, error); /* node-do-not-add-exception-line */
        }
      case 'InvalidSyntax':
        {
          var invalidSyntaxError = new ERR_INVALID_TYPESCRIPT_SYNTAX(error.message);
...
function decorateErrorWithSnippet(error, amaroError) {
  var errorHints = `${amaroError.filename}:${amaroError.startLine}\n${amaroError.snippet}`;
...
function stripTypeScriptModuleTypes(source, filename) {
  ...
  var sourceMap = getOptionValue('--enable-source-maps');
  ...
  // saveCompileCacheEntry().
  var cached = filename ? getCompileCacheEntry(source, filename, kStrippedTypeScript) : undefined;
  if (cached?.transpiled) {
    // TODO(joyeecheung): return Buffer here.
  ...
  var transpiled = processTypeScriptCode(source, options);
  if (cached) {
...
function addSourceMap(code, sourceMap) {
  ...
  var base64SourceMap = Buffer.from(sourceMap).toString('base64');
