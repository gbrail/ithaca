...
var emittedPackageWarnings = new SafeSet();
...
function emitTrailingSlashPatternDeprecation(match, pjsonUrl, base) {
  ...
  var pjsonPath = fileURLToPath(pjsonUrl);
...
function emitInvalidSegmentDeprecation(target, request, match, pjsonUrl, internal, base, isTarget) {
  ...
  var pjsonPath = fileURLToPath(pjsonUrl);
  var double = RegExpPrototypeExec(doubleSlashRegEx, isTarget ? target : request) !== null;
...
function emitLegacyIndexDeprecation(url, path, pkgPath, base, main) {
  ...
  var basePath = fileURLToPath(base);
  if (!main) {
    ...
    process.emitWarning(`Package ${pkgPath} has a "main" field set to "${main}", ` + `excluding the full filename and extension to the resolved file at "${StringPrototypeSlice(path, pkgPath.length)}", imported from ${basePath}.\n Automatic extension resolution of the "main" field is ` + 'deprecated for ES modules.', 'DeprecationWarning', 'DEP0151');
  }
}
var realpathCache = new SafeMap();
var legacyMainResolveExtensions = ['', '.js', '.json', '.node', '/index.js', '/index.json', '/index.node', './index.js', './index.json', './index.node'];
var legacyMainResolveExtensionsIndexes = {
...
function legacyMainResolve(packageJSONUrl, packageConfig, base) {
  ...
  var pkgPath = fileURLToPath(new URL('.', packageJSONUrl));
  var baseStringified = isURL(base) ? base.href : base;
  var resolvedOption = FSLegacyMainResolve(pkgPath, packageConfig.main, baseStringified);
  var maybeMain = resolvedOption <= legacyMainResolveExtensionsIndexes.kResolvedByMainIndexNode ? packageConfig.main || './' : '';
  var resolvedPath = resolve(pkgPath, maybeMain + legacyMainResolveExtensions[resolvedOption]);
  var resolvedUrl = pathToFileURL(resolvedPath);
...
function finalizeResolution(resolved, base, preserveSymlinks) {
  if (RegExpPrototypeExec(encodedSepRegEx, resolved.pathname) !== null) {
    ...
    throw new ERR_INVALID_MODULE_SPECIFIER(resolved.pathname, 'must not include encoded "/" or "\\" characters', basePath);
  }
  var path;
  try {
  ...
  var stats = internalFsBinding.internalModuleStat(StringPrototypeEndsWith(path, '/') ? StringPrototypeSlice(path, -1) : path);

  // Check for stats.isDirectory()
  if (stats === 1) {
    var _basePath;
    try {
      _basePath = fileURLToPath(base);
    } catch {
      _basePath = base;
    }
    throw new ERR_UNSUPPORTED_DIR_IMPORT(path, _basePath, String(resolved));
  } else if (stats !== 0) {
    // Check for !stats.isFile()
    if (process.env.WATCH_REPORT_DEPENDENCIES && process.send) {
      process.send({
        'watch:require': [path || resolved.pathname]
      });
    }
    var _basePath2;
    try {
      _basePath2 = fileURLToPath(base);
    } catch {
      _basePath2 = base;
    }
    throw new ERR_MODULE_NOT_FOUND(path || resolved.pathname, _basePath2, resolved);
  }
  if (!preserveSymlinks) {
    ...
    resolved = pathToFileURL(real + (StringPrototypeEndsWith(path, sep) ? '/' : ''));
...
function throwInvalidSubpath(request, match, packageJSONUrl, internal, base) {
  var reason = `request is not a valid match in pattern "${match}" for the "${internal ? 'imports' : 'exports'}" resolution of ${fileURLToPath(packageJSONUrl)}`;
...
function resolvePackageTargetString(target, subpath, match, packageJSONUrl, base, pattern, internal, isPathMap, conditions) {
  ...
  if (!StringPrototypeStartsWith(target, './')) {
    if (internal && target[0] !== '/' && !StringPrototypeStartsWith(target, '../')) {
      // No need to convert target to string, since it's already presumed to be
      if (!URLCanParse(target)) {
        var exportTarget = pattern ? RegExpPrototypeSymbolReplace(patternRegEx, target, () => subpath) : target + subpath;
  ...
  if (RegExpPrototypeExec(invalidSegmentRegEx, StringPrototypeSlice(target, 2)) !== null) {
    if (RegExpPrototypeExec(deprecatedInvalidSegmentRegEx, StringPrototypeSlice(target, 2)) === null) {
      if (!isPathMap) {
        var request = pattern ? StringPrototypeReplace(match, '*', () => subpath) : match + subpath;
        var resolvedTarget = pattern ? RegExpPrototypeSymbolReplace(patternRegEx, target, () => subpath) : target;
  ...
  var resolved = new URL(target, packageJSONUrl);
  var resolvedPath = resolved.pathname;
  var packagePath = new URL('.', packageJSONUrl).pathname;
  ...
  if (RegExpPrototypeExec(invalidSegmentRegEx, subpath) !== null) {
    var _request = pattern ? StringPrototypeReplace(match, '*', () => subpath) : match + subpath;
    if (RegExpPrototypeExec(deprecatedInvalidSegmentRegEx, subpath) === null) {
      if (!isPathMap) {
        var _resolvedTarget = pattern ? RegExpPrototypeSymbolReplace(patternRegEx, target, () => subpath) : target;
...
function resolvePackageTarget(packageJSONUrl, target, subpath, packageSubpath, base, pattern, internal, isPathMap, conditions) {
  if (typeof target === 'string') {
    ...
    var keys = ObjectGetOwnPropertyNames(target);
    ...
    for (var _i2 = 0; _i2 < keys.length; _i2++) {
      var _key = keys[_i2];
      if (_key === 'default' || conditions.has(_key)) {
        var conditionalTarget = target[_key];
...
function isConditionalExportsMainSugar(exports, packageJSONUrl, base) {
  ...
  var keys = ObjectGetOwnPropertyNames(exports);
...
function packageExportsResolve(packageJSONUrl, packageSubpath, packageConfig, base, conditions) {
  ...
  for (var i = 0; i < keys.length; i++) {
    var key = keys[i];
    var patternIndex = StringPrototypeIndexOf(key, '*');
    if (patternIndex !== -1 && StringPrototypeStartsWith(packageSubpath, StringPrototypeSlice(key, 0, patternIndex))) {
      ...
      var patternTrailer = StringPrototypeSlice(key, patternIndex + 1);
  ...
  if (bestMatch) {
    ...
    var _resolveResult2 = resolvePackageTarget(packageJSONUrl, _target, bestMatchSubpath, bestMatch, base, true, false, StringPrototypeEndsWith(packageSubpath, '/'), conditions);
    if (_resolveResult2 == null) {
      throw exportsNotFound(packageSubpath, packageJSONUrl, base);
    }
...
function patternKeyCompare(a, b) {
  var aPatternIndex = StringPrototypeIndexOf(a, '*');
  var bPatternIndex = StringPrototypeIndexOf(b, '*');
...
function packageImportsResolve(name, base, conditions) {
  ...
  var packageJSONUrl;
  var packageConfig = packageJsonReader.getPackageScopeConfig(base);
  if (packageConfig.exists) {
    ...
    if (imports) {
      if (ObjectPrototypeHasOwnProperty(imports, name) && !StringPrototypeIncludes(name, '*')) {
        ...
        var bestMatch = '';
        var bestMatchSubpath;
        var keys = ObjectGetOwnPropertyNames(imports);
        for (var i = 0; i < keys.length; i++) {
          var key = keys[i];
          var patternIndex = StringPrototypeIndexOf(key, '*');
...
function packageResolve(specifier, base, conditions) {
  ...
  var packageConfig = packageJsonReader.read(packageJSONPath, {
    __proto__: null,
    specifier,
    base,
    isESM: true
...
function moduleResolve(specifier, base, conditions, preserveSymlinks) {
  var protocol = typeof base === 'string' ? StringPrototypeSlice(base, 0, StringPrototypeIndexOf(base, ':') + 1) : base.protocol;
  ...
  if (shouldBeTreatedAsRelativeOrAbsolutePath(specifier)) {
    ...
    var url = URLParse(specifier);
    if (url !== null) {
      resolved = url;
    } else {
      if (isData && !BuiltinModule.canBeRequiredWithoutScheme(specifier)) {
        var _error = new ERR_UNSUPPORTED_RESOLVE_REQUEST(specifier, base);
        setOwnProperty(_error, 'cause', new ERR_INVALID_URL(specifier));
        throw _error;
      }
...
function resolveAsCommonJS(specifier, parentURL) {
  try {
    ...
    var parent = fileURLToPath(parentURL);
    var tmpModule = new CJSModule(parent, null);
    tmpModule.paths = CJSModule._nodeModulePaths(parent);
    var found = CJSModule._resolveFilename(specifier, tmpModule, false);

    // If it is a relative specifier return the relative path
    // to the parent
    if (isRelativeSpecifier(specifier)) {
      var foundURL = pathToFileURL(found).pathname;
      found = relativePosixPath(StringPrototypeSlice(parentURL, 'file://'.length, StringPrototypeLastIndexOf(parentURL, '/')), foundURL);
      ...
      // module
      var i = StringPrototypeIndexOf(specifier, '/');
      var pkg = i === -1 ? specifier : StringPrototypeSlice(specifier, 0, i);
      var needle = `${sep}node_modules${sep}${pkg}${sep}`;
      var index = StringPrototypeLastIndexOf(found, needle);
      if (index !== -1) {
        found = pkg + '/' + ArrayPrototypeJoin(ArrayPrototypeMap(StringPrototypeSplit(StringPrototypeSlice(found, index + needle.length), sep),
...
function defaultResolve(specifier) {
  ...
  var url;
  try {
    url = moduleResolve(specifier, parentURL, conditions, isMain ? getOptionValue('--preserve-symlinks-main') : getOptionValue('--preserve-symlinks'));
  } catch (error) {
...
function decorateErrorWithCommonJSHints(error, specifier, parentURL) {
  var found = resolveAsCommonJS(specifier, parentURL);
  if (found && found !== specifier) {
    // Don't suggest the same input the user provided.
    // Modify the stack and message string to include the hint
    var endOfFirstLine = StringPrototypeIndexOf(error.stack, '\n');
    var hint = `Did you mean to import ${JSONStringify(found)}?`;
    error.stack = StringPrototypeSlice(error.stack, 0, endOfFirstLine) + '\n' + hint + StringPrototypeSlice(error.stack, endOfFirstLine);
    error.message += `\n${hint}`;
  }
}
module.exports = {
