...
function validateKeyOrCertOption(name, value) {
  if (typeof value !== 'string' && !isArrayBufferView(value)) {
    throw new ERR_INVALID_ARG_TYPE(name, ['string', 'Buffer', 'TypedArray', 'DataView'], value);
  }
}
function setKey(context, key, passphrase, name) {
  validateKeyOrCertOption(`${name}.key`, key);
  if (passphrase !== undefined && passphrase !== null) validateString(passphrase, `${name}.passphrase`);
  context.setKey(key, passphrase);
}
function processCiphers(ciphers, name) {
  ciphers = StringPrototypeSplit(ciphers || getDefaultCiphers(), ':');
  var cipherList = ArrayPrototypeJoin(ArrayPrototypeFilter(ciphers, cipher => {
    if (cipher.length === 0) return false;
    if (StringPrototypeStartsWith(cipher, 'TLS_')) return false;
    if (StringPrototypeStartsWith(cipher, '!TLS_')) return false;
    return true;
  }), ':');
  var cipherSuites = ArrayPrototypeJoin(ArrayPrototypeFilter(ciphers, cipher => {
    if (cipher.length === 0) return false;
  ...
  if (cipherSuites === '' && cipherList === '') throw new ERR_INVALID_ARG_VALUE(name, ciphers);
  return {
    cipherList,
    cipherSuites
  };
}
function configSecureContext(context) {
  ...
  if (key) {
    if (ArrayIsArray(key)) {
      for (var i = 0; i < key.length; ++i) {
        var val = key[i];
        var pem = val?.pem !== undefined ? val.pem : val;
        var pass = val?.passphrase !== undefined ? val.passphrase : passphrase;
  ...
  if (sigalgs !== undefined && sigalgs !== null) {
    ...
    if (sigalgs === '') throw new ERR_INVALID_ARG_VALUE(`${name}.sigalgs`, sigalgs);
  ...
  if (privateKeyIdentifier !== undefined && privateKeyIdentifier !== null) {
    ...
    if (typeof privateKeyIdentifier === 'string' && typeof privateKeyEngine === 'string') {
      if (context.setEngineKey) context.setEngineKey(privateKeyIdentifier, privateKeyEngine);else throw new ERR_CRYPTO_CUSTOM_ENGINE_NOT_SUPPORTED();
  ...
  if (dhparam !== undefined && dhparam !== null) {
    ...
    var warning = context.setDHParam(dhparam === 'auto' || dhparam);
    if (warning) process.emitWarning(warning, 'SecurityWarning');
  }
  if (crl !== undefined && crl !== null) {
    if (ArrayIsArray(crl)) {
      for (var _val of crl) {
        validateKeyOrCertOption(`${name}.crl`, _val);
        context.addCRL(_val);
      }
    } else {
  ...
  if (pfx !== undefined && pfx !== null) {
    if (ArrayIsArray(pfx)) {
      ArrayPrototypeForEach(pfx, val => {
        var raw = val.buf || val;
        var pass = val.passphrase || passphrase;
  ...
  if (typeof clientCertEngine === 'string') {
    if (typeof context.setClientCertEngine !== 'function') throw new ERR_CRYPTO_CUSTOM_ENGINE_NOT_SUPPORTED();else context.setClientCertEngine(clientCertEngine);
