...
function readInt32LE(buf) {
  var offset = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  validateNumber(offset, 'offset');
  var first = buf[offset];
  var last = buf[offset + 3];
  if (first === undefined || last === undefined) boundsError(offset, buf.length - 4);
  return first + buf[++offset] * 2 ** 8 + buf[++offset] * 2 ** 16 + (last << 24); // Overflow
}
function readInt24LE(buf) {
  ...
  var last = buf[offset + 2];
  if (first === undefined || last === undefined) boundsError(offset, buf.length - 3);
  var val = first + buf[++offset] * 2 ** 8 + last * 2 ** 16;
  return val | (val & 2 ** 23) * 0x1fe;
}
function readInt16LE(buf) {
  var offset = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  validateNumber(offset, 'offset');
  var first = buf[offset];
  var last = buf[offset + 1];
  if (first === undefined || last === undefined) boundsError(offset, buf.length - 2);
  var val = first + last * 2 ** 8;
...
function readInt24BE(buf) {
  ...
  var last = buf[offset + 2];
  if (first === undefined || last === undefined) boundsError(offset, buf.length - 3);
  var val = first * 2 ** 16 + buf[++offset] * 2 ** 8 + last;
  return val | (val & 2 ** 23) * 0x1fe;
}
function readInt16BE(buf) {
  var offset = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  validateNumber(offset, 'offset');
  var first = buf[offset];
  var last = buf[offset + 1];
  if (first === undefined || last === undefined) boundsError(offset, buf.length - 2);
  var val = first * 2 ** 8 + last;
  return val | (val & 2 ** 15) * 0x1fffe;
}
...
function writeBigU_Int64LE(buf, value, offset, min, max) {
  ...
  var lo = _Number(value & 0xffffffffn);
  buf[offset++] = lo;
  lo = lo >> 8;
  buf[offset++] = lo;
  lo = lo >> 8;
  ...
  lo = lo >> 8;
  buf[offset++] = lo;
  var hi = _Number(value >> 32n & 0xffffffffn);
  buf[offset++] = hi;
  hi = hi >> 8;
  buf[offset++] = hi;
  hi = hi >> 8;
...
function writeU_Int48LE(buf, value, offset, min, max) {
  ...
  var newVal = MathFloor(value * 2 ** -32);
  buf[offset++] = value;
  value = value >>> 8;
  buf[offset++] = value;
  value = value >>> 8;
...
var FastBuffer = /*#__PURE__*/function (_Uint8Array) {
  function FastBuffer() {
    ...
    return _callSuper(this, FastBuffer, arguments);
  }
  _inherits(FastBuffer, _Uint8Array);
  return _createClass(FastBuffer);
}(Uint8Array);
function asciiWrite(buf, string) {
  var offset = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
  var length = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : buf.byteLength - offset;
...
var kDetachKey = _Symbol('detach_key_for_untransferable_arraybuffer');
// This would better be placed in internal/worker/io.js, but that doesn't work
// because Buffer needs this and that would introduce a cyclic dependency.
function markAsUntransferable(obj) {
  if (typeof obj !== 'object' && typeof obj !== 'function' || obj === null) return; // This object is a primitive and therefore already untransferable.
