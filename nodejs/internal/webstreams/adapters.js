...
var kValidateChunk = Symbol('kValidateChunk');
var kDestroyOnSyncError = Symbol('kDestroyOnSyncError');

// Collect all negative (error) ZLIB codes and Z_NEED_DICT
var ZLIB_FAILURES = new SafeSet(ArrayPrototypeFilter(ObjectKeys(zlib), code => code === 'Z_NEED_DICT' || zlib[code] < 0));
...
function handleKnownInternalErrors(cause) {
  ...
  switch (true) {
    ...
    // Falls through
    case causeCode != null && StringPrototypeStartsWith(causeCode, 'ERR__ERROR_'):
      {
        // eslint-disable-next-line no-restricted-syntax
        var error = new TypeError(undefined, {
          cause
        });
...
function newWritableStreamFromStreamWritable(streamWritable) {
  ...
  var checkIfWritableOrOutgoingMessage = streamWritable && typeof streamWritable?.write === 'function' && typeof streamWritable?.on === 'function';
  ...
  var highWaterMark = streamWritable.writableHighWaterMark;
  var strategy = streamWritable.writableObjectMode ? new CountQueuingStrategy({
    highWaterMark
  }) : {
    highWaterMark
  };
  var controller;
  var backpressurePromise;
  var closed;
  function onDrain() {
    backpressurePromise?.resolve();
  }
  var cleanup = eos(streamWritable, error => {
 ...
 * @returns {Writable}
 */
function newStreamWritableFromWritableStream(writableStream) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : kEmptyObject;
  ...
  var writable = new Writable({
    ...
    writev(chunks, callback) {
      ...
      PromisePrototypeThen(writer.ready, () => {
        return PromisePrototypeThen(SafePromiseAllReturnVoid(chunks, data => writer.write(data.chunk)), done, done);
      }, done);
    },
...
var kErrorSentinelAttached = Symbol('kErrorSentinelAttached');
...
function newReadableStreamFromStreamReadable(streamReadable) {
  ...
  var readable = isReadable(streamReadable);
  var objectMode = streamReadable.readableObjectMode;
  if (readable) {
    underlyingSource.pull = function pull() {
      streamReadable.resume();
    };
    var highWaterMark = streamReadable.readableHighWaterMark;
    strategy = isBYOB ? {
      highWaterMark
    } : options.strategy ?? new (objectMode ? CountQueuingStrategy : ByteLengthQueuingStrategy)({
      highWaterMark
    });
  }
  var readableStream = new ReadableStream(underlyingSource, strategy);
 ...
 * @returns {Readable}
 */
function newStreamReadableFromReadableStream(readableStream) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : kEmptyObject;
  ...
  if (encoding !== undefined && !Buffer.isEncoding(encoding)) throw new ERR_INVALID_ARG_VALUE('options.encoding', encoding);
  validateBoolean(objectMode, 'options.objectMode');
...
var emitDEP0201 = getDeprecationWarningEmitter('DEP0201', "Passing 'options.type' to Duplex.toWeb() is deprecated. " + "To specify the ReadableStream type, use 'options.readableType'.", newReadableWritablePairFromDuplex);
...
function newReadableWritablePairFromDuplex(duplex) {
  ...
  if (isDestroyed(duplex)) {
    var _writable = new WritableStream();
    var _readable = new ReadableStream({
      type: readableOptions.type
    });
    _writable.close();
    _readable.cancel();
    return {
  ...
  var writable = isWritable(duplex) ? newWritableStreamFromStreamWritable(duplex, writableOptions) : new WritableStream();
  if (!isWritable(duplex)) writable.close();
  var readable = isReadable(duplex) ? newReadableStreamFromStreamReadable(duplex, readableOptions) : new ReadableStream({
    type: readableOptions.type
  });
...
function newStreamDuplexFromReadableWritablePair() {
  var pair = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : kEmptyObject;
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : kEmptyObject;
  ...
  var duplex = new Duplex({
    ...
    destroy(error, callback) {
      ...
      var closeReader = _async(function () {
        return _invokeIgnored(function () {
          if (!readableClosed) return _awaitIgnored(reader.cancel(error));
        });
      });
      var closeWriter = _async(function () {
        return _invokeIgnored(function () {
          if (!writableClosed) return _awaitIgnored(writer.abort(error));
        });
      });
      if (!writableClosed || !readableClosed) {
        PromisePrototypeThen(SafePromiseAllReturnVoid([closeWriter(), closeReader()]), done, done);
        return;
      }
      done();
    }
...
function newWritableStreamFromStreamBase(streamBase, strategy) {
  ...
  function doWrite(chunk, controller) {
    ...
    if (ret !== 0) promise.reject(new ErrnoException(ret, 'write', req));else if (!req.async) promise.resolve();
    return promise.promise;
  }
  return new WritableStream({
    write(chunk, controller) {
      current = current !== undefined ? PromisePrototypeThen(current, () => doWrite(chunk, controller), error => controller.error(error)) : doWrite(chunk, controller);
      return current;
    },
    close() {
      var promise = PromiseWithResolvers();
      ...
      var err = streamBase.shutdown(req);
...
function newReadableStreamFromStreamBase(streamBase, strategy) {
  ...
  if (typeof streamBase.onread === 'function') throw new ERR_INVALID_STATE('StreamBase already has a consumer');
