...
var getCryptoKeyHandle;
var getCryptoKeyType;
var getCryptoKeyExtractable;
var getCryptoKeyAlgorithm;
var getCryptoKeyUsagesMask;
var getKeyObjectHandle;
var getKeyObjectType;
var kStrict = 2;
var kStrictWithoutPrototypes = 3;
var kLoose = 0;
var kPartial = 1;
var kNoIterator = 0;
var kIsArray = 1;
...
function isPartialUint8Array(a, b) {
  var lenA = getByteLength(a);
  var lenB = getByteLength(b);
  if (lenA < lenB) {
    return false;
  }
  var offsetA = 0;
  for (var offsetB = 0; offsetB < lenB; offsetB++) {
...
function isEnumerableOrIdentical(val1, val2, prop, mode, memos) {
  ...
  // This is handled by Object.keys()
...
function objectComparisonStart(val1, val2, mode, memos) {
  ...
  if (ArrayIsArray(val1)) {
    if (!ArrayIsArray(val2) || val1.length !== val2.length && (mode !== kPartial || val1.length < val2.length) || hasUnequalTag(val1, val2)) {
      return false;
    }
    var filter = mode !== kLoose ? ONLY_ENUMERABLE : ONLY_ENUMERABLE | SKIP_SYMBOLS;
  ...
  var val1Tag;
  if (val1[SymbolToStringTag] === undefined && (val1Tag = ObjectPrototypeToString(val1)) === '[object Object]') {
    if (slowHasUnequalTag(val1Tag, val1, val2)) {
      return false;
    }
    return keyCheck(val1, val2, mode, memos, kNoIterator);
  } else if (isSet(val1)) {
    if (!isSet(val2) || val1.size !== val2.size && (mode !== kPartial || val1.size < val2.size) || hasUnequalTag(val1, val2)) {
      return false;
    }
    return keyCheck(val1, val2, mode, memos, kIsSet);
  } else if (isMap(val1)) {
    if (!isMap(val2) || val1.size !== val2.size && (mode !== kPartial || val1.size < val2.size) || hasUnequalTag(val1, val2)) {
      return false;
    }
    ...
    // the symbols.
    var _filter = mode !== kLoose ? ONLY_ENUMERABLE : ONLY_ENUMERABLE | SKIP_SYMBOLS;
    var _keys = getOwnNonIndexProperties(val2, _filter);
    if (mode !== kPartial && _keys.length !== getOwnNonIndexProperties(val1, _filter).length) {
      return false;
    }
    return keyCheck(val1, val2, mode, memos, kNoIterator, _keys);
  } else if (isDate(val1)) {
    if (!isDate(val2) || hasUnequalTag(val1, val2)) {
      return false;
    }
    var time1 = DatePrototypeGetTime(val1);
    var time2 = DatePrototypeGetTime(val2);
    // eslint-disable-next-line no-self-compare
  ...
  } else if (slowHasUnequalTag(val1Tag ?? ObjectPrototypeToString(val1), val1, val2) || ArrayIsArray(val2) || isArrayBufferView(val2) || isSet(val2) || isMap(val2) || isDate(val2) || isRegExp(val2) || isAnyArrayBuffer(val2)) {
    return false;
    ...
    var hasOwnVal2Cause = hasOwn(val2, 'cause');
    ...
    if (!isKeyObject(val2) || getKeyObjectType(val1) !== getKeyObjectType(val2) || !getKeyObjectHandle(val1).equals(getKeyObjectHandle(val2))) {
      return false;
    }
    ...
    if (!isCryptoKey(val2) || getCryptoKeyType(val1) !== getCryptoKeyType(val2) || getCryptoKeyExtractable(val1) !== getCryptoKeyExtractable(val2) || !innerDeepEqual(getCryptoKeyAlgorithm(val1), getCryptoKeyAlgorithm(val2), mode, memos) || getCryptoKeyUsagesMask(val1) !== getCryptoKeyUsagesMask(val2) || !getCryptoKeyHandle(val1).equals(getCryptoKeyHandle(val2))) {
      return false;
    }
  } else if (isBoxedPrimitive(val2) || isNativeError(val2) || val2 instanceof Error || isWeakMap(val1) || isWeakSet(val1) || isPromise(val1)) {
    return false;
  }
  return keyCheck(val1, val2, mode, memos, kNoIterator);
}
function partialSymbolEquiv(val1, val2, keys2) {
  var symbolKeys = getOwnSymbols(val2);
...
function keyCheck(val1, val2, mode, memos, iterationType, keys2) {
  ...
  var isArrayLikeObject = keys2 !== undefined;
  if (keys2 === undefined) {
  ...
  if (!isArrayLikeObject) {
    ...
    if (mode === kPartial) {
      ...
      for (var key of getOwnSymbols(val1)) {
        if (hasEnumerable(val1, key)) {
          ArrayPrototypePush(keys1, key);
        }
      }
      for (var _key of getOwnSymbols(val2)) {
        if (hasEnumerable(val2, _key)) {
          ArrayPrototypePush(keys2, _key);
        }
      }
...
function handleCycles(val1, val2, mode, keys1, keys2, memos, iterationType) {
  ...
  if (memos.set === undefined) {
    if (memos.deep === false) {
      ...
      memos.d = val2;
      memos.deep = true;
      var result = objEquiv(val1, val2, mode, keys1, keys2, memos, iterationType);
      memos.deep = false;
      if (memos.set !== undefined) {
  ...
  var originalSize = set.size;
  set.add(val1);
  set.add(val2);
  if (originalSize !== set.size - 2) {
    return originalSize === set.size;
  }
  var areEq = objEquiv(val1, val2, mode, keys1, keys2, memos, iterationType);
  set.delete(val1);
  set.delete(val2);
  return areEq;
}
...
function setObjectEquiv(array, a, b, mode, memo) {
  ...
  var start = 0;
  var end = array.length - 1;
  var comparator = mode !== kLoose ? objectComparisonStart : innerDeepEqual;
  var extraChecks = mode === kLoose || array.length !== a.size;
  for (var val1 of a) {
    if (extraChecks) {
...
function compareSmallSets(a, b, val, iteratorB, mode, memo) {
  ...
  var firstA = iteratorA.next().value;
  var first = innerDeepEqual(firstA, val, mode, memo);
  if (first) {
    if (b.size === 1) {
      // Partial mode && a.size === 1 || b.size === 1
      return true;
    }
    var secondA = iteratorA.next().value;
    return b.has(secondA) || innerDeepEqual(secondA, iteratorB.next().value, mode, memo);
  }
  return a.size !== 1 && innerDeepEqual(iteratorA.next().value, val, mode, memo) && (b.size === 1 ||
  // Partial mode
  b.has(firstA) ||
  // Primitive or reference equal
...
function partialObjectMapEquiv(array, a, b, mode, memo) {
  ...
  for (var {
    ...
    if (typeof key1 === 'object' && key1 !== null) {
      var innerStart = start;
      if (direction === 1) {
        var _key2 = array[start];
        if (objectComparisonStart(key1, _key2, mode, memo) && innerDeepEqual(item1, b.get(_key2), mode, memo)) {
          if (start === end) {
            return true;
          }
          start += 1;
...
function mapObjectEquiv(array, a, b, mode, memo) {
  ...
  for (var {
    ...
    var innerStart = start;
    if (direction === 1) {
      var _key3 = array[start];
      if (comparator(key1, _key3, mode, memo) && innerDeepEqual(item1, b.get(_key3), mode, memo)) {
        start += 1;
        continue;
      }
      if (start === end) {
...
function mapEquiv(a, b, mode, memo) {
  ...
  for (var {
    ...
    if (typeof key2 === 'object' && key2 !== null) {
      ...
      var _item = a.get(key2);
      if (_item === undefined && !a.has(key2) || !innerDeepEqual(_item, item2, mode, memo)) {
...
function partialSparseArrayEquiv(a, b, mode, memos, startA, startB) {
  ...
  var lenA = keysA.length - startA;
  var lenB = keysB.length - startB;
  if (lenA < lenB) {
    return false;
  }
  for (var i = 0; i < lenB; i++) {
    var keyB = keysB[startB + i];
...
function partialArrayEquiv(a, b, mode, memos) {
  ...
  for (var i = 0; i < b.length; i++) {
    var isSparse = b[i] === undefined && !hasOwn(b, i);
    if (isSparse) {
...
function objEquiv(a, b, mode, keys1, keys2, memos, iterationType) {
  ...
  if (keys2.length > 0) {
    ...
    for (; i < keys2.length; i++) {
      ...
      var descriptor = ObjectGetOwnPropertyDescriptor(a, _key4);
      if (descriptor === undefined || descriptor.enumerable !== true) {
        return false;
      }
      var value = descriptor.writable !== undefined ? descriptor.value : a[_key4];
      if (!innerDeepEqual(value, b[_key4], mode, memos)) {
        return false;
      }
    }
  }
  if (iterationType === kIsArray) {
    ...
    for (var _i = 0; _i < a.length; _i++) {
      if (b[_i] === undefined) {
        if (!hasOwn(b, _i)) return sparseArrayEquiv(a, b, mode, memos, _i);
        if ((a[_i] !== undefined || !hasOwn(a, _i)) && (mode !== kLoose || a[_i] !== null)) return false;
      } else if ((a[_i] === undefined || !innerDeepEqual(a[_i], b[_i], mode, memos)) && (mode !== kLoose || b[_i] !== null)) {
        return false;
      }
    }
...
var detectCycles = function (val1, val2, mode) {
  try {
