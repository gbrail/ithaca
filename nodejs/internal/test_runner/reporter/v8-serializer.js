...
module.exports = function v8Reporter(source) {
  return new _AsyncGenerator(function (_generator) {
    var serializer = new DefaultSerializer();
    serializer.writeHeader();
    var headerLength = TypedArrayPrototypeGetLength(serializer.releaseBuffer());
    return _continueIgnored(_forAwaitOf(source, function (item) {
      var originalError = item.data.details?.error;
      if (originalError) {
      ...
      var serializedMessage = serializer.releaseBuffer();
      var serializedMessageLength = serializedMessage.length - (4 + headerLength);
      serializedMessage.set([serializedMessageLength >> 24 & 0xFF, serializedMessageLength >> 16 & 0xFF, serializedMessageLength >> 8 & 0xFF, serializedMessageLength & 0xFF], headerLength);
      return _generator._yield(serializedMessage).then(_empty);
    }));
  });
};
