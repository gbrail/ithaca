...
var report = {
  ...
  getReport(err) {
    if (err === undefined) err = new ERR_SYNTHETIC();else validateObject(err, 'err');
...
function addSignalHandler(sig) {
  if (nr.shouldReportOnSignal()) {
    if (typeof sig !== 'string') sig = nr.getSignal();
    process.on(sig, signalHandler);
  }
}
function removeSignalHandler() {
  var sig = nr.getSignal();
  if (sig) process.removeListener(sig, signalHandler);
}
