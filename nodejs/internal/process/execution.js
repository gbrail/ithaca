...
function evalScript(name, body, breakFirstLine, print) {
  var shouldLoadESM = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
  var origModule = globalThis.module; // Set e.g. when called from the REPL.
  var module = createModule(name);
  var baseUrl = pathToFileURL(module.filename).href;
  if (shouldUseModuleEntryPoint(name, body)) {
    return getOptionValue('--strip-types') ? evalTypeScriptModuleEntryPoint(body, print) : evalModuleEntryPoint(body, print);
  }
  var evalFunction = () => runScriptInContext(name, body, breakFirstLine, print, module, baseUrl, undefined, origModule);
  if (shouldLoadESM) {
    return require('internal/modules/run_main').runEntryPointWithESMLoader(evalFunction);
  }
  evalFunction();
}
var exceptionHandlerState = {
  captureFn: null,
  // Primary callback (for domain's exclusive use)
  auxiliaryCallbacks: [],
  // Auxiliary callbacks (for REPL, etc.) - always called
  reportFlag: false
};
...
function addUncaughtExceptionCaptureCallback(fn) {
  ...
  if (exceptionHandlerState.auxiliaryCallbacks.length === 0 && exceptionHandlerState.captureFn === null) {
    exceptionHandlerState.reportFlag = process.report.reportOnUncaughtException === true;
    process.report.reportOnUncaughtException = false;
    shouldAbortOnUncaughtToggle[0] = 0;
  }
...
function createOnGlobalUncaughtException() {
  ...
  return (er, fromPromise) => {
    ...
    if (exceptionHandlerState.captureFn !== null) {
      ...
      var handled = false;
      for (var i = exceptionHandlerState.auxiliaryCallbacks.length - 1; i >= 0; i--) {
    ...
    if (afterHooksExist()) {
      do {
        ...
        if (asyncId === 0) popAsyncContext(0);else emitAfter(asyncId);
...
function decorateCJSErrorWithTSMessage(originalStack, newMessage) {
  ...
  return StringPrototypeSlice(originalStack, 0, index) + '\n' + newMessage + StringPrototypeSlice(originalStack, index);
}
 ...
 * @returns {void}
 */
function evalTypeScript(name, source, breakFirstLine, print) {
  var shouldLoadESM = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
  var origModule = globalThis.module; // Set e.g. when called from the REPL.
  var module = createModule(name);
  var baseUrl = pathToFileURL(module.filename).href;
  ...
  var evalFunction = () => runScriptInContext(name, sourceToRun, breakFirstLine, print, module, baseUrl, compiledScript, origModule);
  if (shouldLoadESM) {
    return require('internal/modules/run_main').runEntryPointWithESMLoader(evalFunction);
  }
  evalFunction();
...
function evalTypeScriptModuleEntryPoint(source, print) {
  ...
  return require('internal/modules/run_main').runEntryPointWithESMLoader(_async(function (loader) {
    var url = getEvalModuleUrl();
    var moduleWrap;
    try {
      ...
      try {
        var strippedSource = stripTypeScriptModuleTypes(source, kEvalTag);
 ...
 * @returns {void}
 */
function parseAndEvalCommonjsTypeScript(name, source, breakFirstLine, print) {
  var shouldLoadESM = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
  // We know its a TypeScript module, we can safely emit the experimental warning.
  var strippedSource = stripTypeScriptModuleTypes(source, kEvalTag);
...
function compileScript(name, body, baseUrl) {
  var importModuleDynamically = _async(function (specifier, _, importAttributes, phase) {
    ...
    return cascadedLoader.import(specifier, baseUrl, importAttributes, phase === 'source' ? kSourcePhase : kEvaluationPhase);
  });
  var hostDefinedOptionId = Symbol(name);
  return makeContextifyScript(body,
  // code
  name,
  // filename,
  0,
...
function runScriptInContext(name, body, breakFirstLine, print, module, baseUrl, compiledScript, origModule) {
  ...
  var result = module._compile(script, `${name}-wrapper`)(() => {
    ...
    return runScriptInThisContext(compiledScript ?? compileScript(name, body, baseUrl), true, !!breakFirstLine);
  });
  if (print) {
    var {
      log
    } = require('internal/console/global');
    var printResult = () => log(result);
