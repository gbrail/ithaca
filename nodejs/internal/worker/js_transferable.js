...
function setup() {
  ...
  setDeserializerCreateObjectFunction(deserializeInfo => {
    var {
      0: module,
      1: ctor
    } = StringPrototypeSplit(deserializeInfo, ':', 2);
    var Ctor = require(module)[ctor];
 ...
 */
function markTransferMode(obj) {
  var cloneable = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  var transferable = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  if (typeof obj !== 'object' && typeof obj !== 'function' || obj === null) return; // This object is a primitive and therefore already untransferable.
  var mode = kDisallowCloneAndTransfer;
  if (cloneable) mode |= kCloneable;
  if (transferable) mode |= kTransferable;
  obj[transfer_mode_private_symbol] = mode;
}
webidl.converters.StructuredSerializeOptions = webidl.createDictionaryConverter('StructuredSerializeOptions', [{
  key: 'transfer',
...
function structuredClone(value, options) {
  ...
  var idlOptions = webidl.converters.StructuredSerializeOptions(options, {
    __proto__: null,
    prefix: "Failed to execute 'structuredClone'",
    context: 'Options'
  });
  var serializedData = nativeStructuredClone(value, idlOptions);
