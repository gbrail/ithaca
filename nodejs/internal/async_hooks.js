...
var emitPromiseResolveNative = emitHookFactory(promise_resolve_symbol, 'emitPromiseResolveNative');
var domain_cb;
function useDomainTrampoline(fn) {
  domain_cb = fn;
}
function callbackTrampoline(asyncId, resource, cb) {
  var index = async_hook_fields[kStackLength] - 1;
...
function executionAsyncResource() {
  ...
  var resource = execution_async_resources[index] || executionAsyncResource_(index);
...
function lookupPublicResource(resource) {
  ...
  // AsyncWrap instances.
  var publicResource = resource[resource_symbol];
  ...
  try {
    ...
    for (var i = 0; i < active_hooks.array.length; i++) {
      if (typeof active_hooks.array[i][init_symbol] === 'function') {
        if (isPromiseHook && active_hooks.array[i][kNoPromiseHook]) {
          continue;
        }
...
function emitHookFactory(symbol, name) {
  var fn = emitHook.bind(undefined, symbol);
...
function trackPromise(promise, parent) {
  ...
  var triggerAsyncId = parent ? getOrSetAsyncId(parent) : getDefaultTriggerAsyncId();
...
function promiseInitHook(promise, parent) {
  ...
  var asyncId = promise[async_id_symbol];
  var triggerAsyncId = promise[trigger_async_id_symbol];
...
function promiseBeforeHook(promise) {
  ...
  var asyncId = promise[async_id_symbol];
  var triggerId = promise[trigger_async_id_symbol];
...
function defaultTriggerAsyncIdScope(triggerAsyncId, block) {
  ...
  if (triggerAsyncId === undefined) return ReflectApply(block, null, args);
...
function emitInitScript(asyncId, type, triggerAsyncId, resource) {
  var isPromiseHook = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
...
function emitBeforeScript(asyncId, triggerAsyncId, resource) {
  var isPromiseHook = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
...
function pushAsyncContext(asyncId, triggerAsyncId, resource) {
  ...
  if (offset * 2 >= async_wrap.async_ids_stack.length) return pushAsyncContext_(asyncId, triggerAsyncId);
...
var kNoPromiseHook = Symbol('kNoPromiseHook');
module.exports = {
  executionAsyncId,
  triggerAsyncId,
  // Private API
