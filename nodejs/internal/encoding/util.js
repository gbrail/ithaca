...
function unfinishedBytesUtf8(data, len) {
  ...
  var lower = lead === 0xf0 ? 0x90 : lead === 0xe0 ? 0xa0 : 0x80;
  var upper = lead === 0xf4 ? 0x8f : lead === 0xed ? 0x9f : 0xbf;
  var next = data[len - pos];
...
function mergePrefixUtf8(data, chunk) {
  ...
  if (data.length < 3) {
    ...
    var res = new Uint8Array(data.length + chunk.length);
    res.set(chunk);
  ...
  var temp = new Uint8Array(chunk.length + 3); // We have 1-3 bytes and need 1-3 more bytes
  temp.set(chunk);
  ...
  for (var i = 1; i <= 3; i++) {
    var unfinished = unfinishedBytesUtf8(temp, chunk.length + i); // 0-3
    if (unfinished <= i) {
      // Always reachable at 3, but we still need 'unfinished' value for it
      var add = i - unfinished; // 0-3
