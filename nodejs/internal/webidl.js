...
var BIGINT_2_63 = 1n << 63n;
var BIGINT_2_64 = 1n << 64n;
var converters = {
  __proto__: null
};
 ...
 * @returns {TypeError}
 */
function makeException(message) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : kEmptyObject;
  var prefix = options.prefix ? options.prefix + ': ' : '';
  var context = options.context?.length === 0 ? '' : (options.context ?? 'Value') + ' ';
  return codedTypeError(`${prefix}${context}${message}`, options.code || 'ERR_INVALID_ARG_TYPE');
}
...
function makeOptions(options) {
  var context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : options.context;
  var code = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : options.code;
  return {
...
function evenRound(x) {
  ...
  var i = integerPart(x);
  var remainder = MathAbs(x % 1);
  var sign = MathSign(x);
  if (remainder === 0.5) {
    return i % 2 === 0 ? i : i + sign;
  }
  var r = remainder < 0.5 ? i : i + sign;
  if (r === 0) {
    return 0;
  }
  return r;
...
function toNumber(V) {
  ...
  if (typeof V === 'bigint') {
    ...
    throw makeException('is a BigInt and cannot be converted to a number.', options);
  }
  if (typeof V === 'symbol') {
    // ECMA-262 ToNumber step 2: Symbol values throw.
    throw makeException('is a Symbol and cannot be converted to a number.', options);
  }
...
function toString(V) {
  ...
  if (typeof V === 'symbol') {
    ...
    throw makeException('is a Symbol and cannot be converted to a string.', options);
  }
 ...
 * @returns {number}
 */
function convertToInt(V, bitLength) {
  var signedness = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'unsigned';
  var options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : kEmptyObject;
  var signed = signedness === 'signed';
  var upperBound;
  var lowerBound;
  ...
  if (typeof V === 'number') {
    ...
    if (options !== kEmptyObject && options.enforceRange) {
      ...
      if (!NumberIsFinite(V)) {
        throw makeException('is not a finite number.', options);
      }
      var _integer = integerPart(V);
      if (_integer < lowerBound || _integer > upperBound) {
        throw makeException(`is outside the expected range of ${lowerBound} to ${upperBound}.`, makeOptions(options, options.context, 'ERR_OUT_OF_RANGE'));
      }
      return _integer;
    }
  ...
  var twoToTheBitLength = pow2(bitLength);
 ...
 * @returns {Converter}
 */
function createIntegerConverter(bitLength) {
  var signedness = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'unsigned';
  return function (V) {
    var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : kEmptyObject;
 ...
 */
converters.object = function (V) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : kEmptyObject;
  // Web IDL object step 1: throw unless V is an ECMA-262 Object.
  if (type(V) !== 'Object') {
    throw makeException('is not an object.', options);
  }
 ...
 * @returns {void}
 */
function requiredArguments(length, required) {
  var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : kEmptyObject;
  if (length < required) {
    throw makeException(`${required} argument${required === 1 ? '' : 's'} required, but only ${length} present.`, makeOptions(options, '', 'ERR_MISSING_ARGS'));
  }
}
...
function createEnumConverter(name, values) {
  var E = new SafeSet(new SafeArrayIterator(values));
  return function (V) {
    var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : kEmptyObject;
    // Web IDL enumeration step 1: convert V with ToString.
    var S = toString(V, options);

    // Step 2: throw unless S is one of the enumeration values.
    if (!E.has(S)) {
      throw makeException(`'${S}' is not a valid enum value of type ${name}.`, makeOptions(options, options.context, 'ERR_INVALID_ARG_VALUE'));
    }
...
function missingDictionaryMemberMessage(dictionaryName, key) {
  return `cannot be converted to '${dictionaryName}' because ` + `'${key}' is required in '${dictionaryName}'.`;
}
 ...
 * @returns {Converter}
 */
function createDictionaryConverter(dictionaryName, members) {
  var compareMembers = (a, b) => {
    if (a.key === b.key) {
      return 0;
    }
    return a.key < b.key ? -1 : 1;
  };
  var dictionaries = ArrayIsArray(members[0]) ? members : [members];
  ...
  for (var i = 0; i < dictionaries.length; i++) {
    ArrayPrototypePush(sortedDictionaries, ArrayPrototypeToSorted(dictionaries[i], compareMembers));
  }
  return function (jsDict) {
    var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : kEmptyObject;
    // Step 1: reject non-object, non-null, non-undefined values.
    if (jsDict != null && type(jsDict) !== 'Object') {
      throw makeException('cannot be converted to a dictionary', options);
    }
    ...
    for (var _i = 0; _i < sortedDictionaries.length; _i++) {
      var sortedMembers = sortedDictionaries[_i];
      for (var j = 0; j < sortedMembers.length; j++) {
        ...
        var jsMemberValue = jsDict == null ? undefined : jsDict[key];
        ...
        if (jsMemberValue !== undefined) {
          ...
          var idlMemberValue = converter(jsMemberValue, makeOptions(options, dictionaryMemberContext(key, options)));
          ...
          throw makeException(missingDictionaryMemberMessage(dictionaryName, key), makeOptions(options, options.context, 'ERR_MISSING_OPTION'));
        }
      }
    }
...
function createSequenceConverter(converter) {
  return function (V) {
    ...
    if (type(V) !== 'Object') {
      throw makeException('cannot be converted to sequence.', options);
    }

    // Step 2: GetMethod(V, %Symbol.iterator%).
    var method = V[SymbolIterator];
    ...
    var iterator = FunctionPrototypeCall(method, V);
    ...
    while (true) {
      ...
      var next = FunctionPrototypeCall(nextMethod, iterator);
      if (type(next) !== 'Object') {
        throw makeException('cannot be converted to sequence.', options);
      }
      ...
      var idlValue = converter(next.value, makeOptions(options, sequenceElementContext(idlSequence.length, options)));
...
function getViewedArrayBuffer(V) {
  ...
  return isTypedArray(V) ? TypedArrayPrototypeGetBuffer(V) : DataViewPrototypeGetBuffer(V);
}
...
function validateBufferSourceBacking(buffer, options) {
  ...
  try {
    ...
    if (!options.allowShared) {
      throw makeException('is a view on a SharedArrayBuffer, which is not allowed.', options);
    }
 ...
 * @returns {Uint8Array}
 */
converters.Uint8Array = function (V) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : kEmptyObject;
  // Typed array conversion steps 1-2: T is Uint8Array, and V must
  // have [[TypedArrayName]] equal to "Uint8Array".
  if (!ArrayBufferIsView(V) || TypedArrayPrototypeGetSymbolToStringTag(V) !== 'Uint8Array') {
    throw makeException('is not an Uint8Array object.', options);
  }
...
converters.BufferSource = function (V) {
  ...
  if (ArrayBufferIsView(V)) {
    var buffer = getViewedArrayBuffer(V);
    ...
    try {
      ...
      _resizable = ArrayBufferPrototypeGetResizable(buffer);
    } catch {
      throw makeException('is a view on a SharedArrayBuffer, which is not allowed.', options);
    }
    if (_resizable && !options.allowResizable) {
      throw makeException('is backed by a resizable ArrayBuffer, which is not allowed.', options);
    }
    return V;
  }
 ...
 */
converters.AllowSharedBufferSource = function (V) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : kEmptyObject;
  ...
  if (ArrayBufferIsView(V)) {
    ...
    var _resizable2;
    try {
      _resizable2 = ArrayBufferPrototypeGetResizable(buffer);
    } catch {
      validateAllowGrowableSharedArrayBuffer(buffer, options);
      return V;
    }
    validateAllowResizableArrayBuffer(_resizable2, options);
    return V;
  }
  var resizable;
  try {
    ...
    throw makeException('is not instance of ArrayBuffer, SharedArrayBuffer, Buffer, ' + 'TypedArray, or DataView.', options);
  }
  validateAllowResizableArrayBuffer(resizable, options);
  return V;
};
