...
function prepareStackTraceWithSourceMaps(error, trace) {
  ...
  var lastSourceMap;
  var lastFileName;
  var preparedTrace = ArrayPrototypeJoin(ArrayPrototypeMap(trace, (callSite, i) => {
    try {
      ...
      var sm = fileName === lastFileName ? lastSourceMap : findSourceMap(fileName);
...
function serializeJSStackFrame(sm, callSite, callerCallSite) {
  ...
  if (originalSource === undefined || originalLine === undefined || originalColumn === undefined) {
    return `${callSite}`;
  }
  var name = getOriginalSymbolName(sm, callSite, callerCallSite);
  var originalSourceNoScheme = StringPrototypeStartsWith(originalSource, 'file://') ? fileURLToPath(originalSource) : originalSource;
  // Construct call site name based on: v8.dev/docs/stack-trace-api:
  var fnName = callSite.getFunctionName() ?? callSite.getMethodName();
  var prefix = '';
  if (callSite.isAsync()) {
  ...
  var namePrefix = typeName !== null && typeName !== 'global' ? `${typeName}.` : '';
  var originalName = `${fnName || '<anonymous>'}`;
  var mappedName = `${namePrefix}${name || originalName}` || '';
  // Replace the transpiled call site with the original:
  return `${prefix}${mappedName} (` + `${originalSourceNoScheme}:${originalLine + 1}:` + `${originalColumn + 1})`;
}
...
function getOriginalSymbolName(sourceMap, callSite, callerCallSite) {
  ...
  var enclosingEntry = sourceMap.findEntry(callSite.getEnclosingLineNumber() - 1, callSite.getEnclosingColumnNumber() - 1);
  if (enclosingEntry.name) return enclosingEntry.name;
  // Fallback to using the symbol name attached to the caller site:
  var currentFileName = callSite.getFileName();
  if (callerCallSite && currentFileName === callerCallSite.getFileName()) {
    var {
      name
    } = sourceMap.findEntry(callerCallSite.getLineNumber() - 1, callerCallSite.getColumnNumber() - 1);
    return name;
  }
}
...
function getErrorSource(sourceMap, originalSourcePath, originalLine, originalColumn) {
  var line = getSourceLine(sourceMap, originalSourcePath, originalLine);
  if (!line) {
    return;
  }
  var originalSourcePathNoScheme = StringPrototypeStartsWith(originalSourcePath, 'file://') ? fileURLToPath(originalSourcePath) : originalSourcePath;

  // Display ^ in appropriate position, regardless of whether tabs or
  // spaces are used:
  var prefix = '';
  for (var character of new SafeStringIterator(StringPrototypeSlice(line, 0, originalColumn + 1))) {
  ...
  var exceptionLine = `${originalSourcePathNoScheme}:${originalLine + 1}\n${line}\n${prefix}^\n`;
...
function getSourceMapErrorSource(fileName, lineNumber, columnNumber) {
  ...
  var errorSource = getErrorSource(sm, originalSource, originalLine, originalColumn);
  return errorSource;
}
