...
var getModuleSourceMapCache = getLazy(() => {
  var {
    SourceMapCacheMap
  } = require('internal/source_map/source_map_cache_map');
...
var generatedSourceMapCache = new SafeMap();
var kLeadingProtocol = /^\w+:\/\//;
 ...
 */
function setSourceMapsSupport(enabled) {
...
function extractSourceURLMagicComment(content) {
  ...
  var sourceURL = matchSourceURL.groups.sourceURL;
...
function maybeCacheSourceMap(filename, content, moduleInstance, isGeneratedSource, sourceURL, sourceMapURL) {
  var support = getSourceMapsSupport();
  ...
  var data = dataFromUrl(filename, sourceMapURL);
  ...
  // context.
  var keys = sourceURL ? [filename, sourceURL] : [filename];
...
function maybeCacheGeneratedSourceMap(content) {
  ...
  var sourceURL = extractSourceURLMagicComment(content);
  if (sourceURL === null) {
    return;
  }
  var sourceMapURL = extractSourceMapURLMagicComment(content);
...
function dataFromUrl(sourceURL, sourceMappingURL) {
  var url = URLParse(sourceMappingURL);
  if (url != null) {
    switch (url.protocol) {
      case 'data:':
  ...
  var mapURL = URLParse(sourceMappingURL, sourceURL);
  if (mapURL === null) {
    return null;
  }
...
function lineLengths(content) {
  ...
  for (var i = 0; i < contentLength; i++, lineLength++) {
    var codePoint = StringPrototypeCodePointAt(content, i);
...
function sourceMapFromFile(mapURL) {
  try {
    var content = readFileSync(fileURLToPath(mapURL), 'utf8');
...
function sourceMapFromDataUrl(sourceURL, url) {
  var {
    0: format,
    1: data
  } = StringPrototypeSplit(url, ',', 2);
  var splitFormat = StringPrototypeSplit(format, ';');
  var contentType = splitFormat[0];
  var base64 = splitFormat[splitFormat.length - 1] === 'base64';
  if (contentType === 'application/json') {
    var decodedData = base64 ? Buffer.from(data, 'base64').toString('utf8') : data;
    try {
      var parsedData = JSONParse(decodedData);
...
function sourceMapCacheToObject() {
  var moduleSourceMapCache = getModuleSourceMapCache();
...
function findSourceMap(sourceURL) {
  ...
  try {
    ...
    var entry = getModuleSourceMapCache().get(sourceURL) ?? generatedSourceMapCache.get(sourceURL);
    if (entry?.data == null) {
      return undefined;
    }
    var sourceMap = entry.sourceMap;
    if (sourceMap === undefined) {
      sourceMap = new SourceMap(entry.data, {
        lineLengths: entry.lineLengths
      });
...
function getOriginalSource(payload, originalSourcePath) {
  ...
  var sourceContentIndex = ArrayPrototypeIndexOf(payload.sources, originalSourcePath);
  if (payload.sourcesContent?.[sourceContentIndex]) {
    ...
    var originalSourcePathNoScheme = fileURLToPath(originalSourcePath);
    try {
 ...
 */
function getSourceLine(sourceMap, originalSourcePath, originalLine) {
  var source = getOriginalSource(sourceMap.payload, originalSourcePath);
  if (typeof source !== 'string') {
    return;
  }
  var lines = RegExpPrototypeSymbolSplit(/\r?\n/, source, originalLine + 1);
  var line = lines[originalLine];
  return line;
}
module.exports = {
  findSourceMap,
