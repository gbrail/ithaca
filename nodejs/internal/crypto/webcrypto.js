...
function callSubtleCryptoMethod(fn, receiver, args) {
  try {
    var result = ReflectApply(fn, receiver, args);
    ...
    var shouldCleanupResult = prepareWebCryptoResult(result);
    try {
      return PromiseResolve(result);
    } finally {
      if (shouldCleanupResult) cleanupWebCryptoResult(result);
    }
  } catch (err) {
...
function digestImpl(algorithm, data) {
  ...
  var prefix = "Failed to execute 'digest' on 'SubtleCrypto'";
  ...
  var normalizedAlgorithm = normalizeAlgorithm(algorithm, 'digest');
...
function generateKeyImpl(algorithm, extractable, keyUsages) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'generateKey' on 'SubtleCrypto'";
  ...
  var usages = webidl.converters['sequence<KeyUsage>'](keyUsages, {
    prefix,
    context: '3rd argument'
  });
  var normalizedAlgorithm = normalizeAlgorithm(algorithm, 'generateKey');
  switch (normalizedAlgorithm.name) {
    ...
    // Fall through
    case 'RSA-OAEP':
      return require('internal/crypto/rsa').rsaKeyGenerate(normalizedAlgorithm, extractable, usages);
    case 'Ed25519':
    // Fall through
    case 'Ed448':
    // Fall through
    ...
    // Fall through
    case 'X448':
      return require('internal/crypto/cfrg').cfrgGenerateKey(normalizedAlgorithm, extractable, usages);
    case 'ECDSA':
    // Fall through
    case 'ECDH':
      return require('internal/crypto/ec').ecGenerateKey(normalizedAlgorithm, extractable, usages);
    case 'HMAC':
      return require('internal/crypto/mac').hmacGenerateKey(normalizedAlgorithm, extractable, usages);
    case 'AES-CTR':
    // Fall through
    case 'AES-CBC':
    // Fall through
    ...
    // Fall through
    case 'AES-KW':
      return require('internal/crypto/aes').aesGenerateKey(normalizedAlgorithm, extractable, usages);
    case 'ChaCha20-Poly1305':
      return require('internal/crypto/chacha20_poly1305').c20pGenerateKey(normalizedAlgorithm, extractable, usages);
    case 'ML-DSA-44':
    // Fall through
    case 'ML-DSA-65':
    // Fall through
    case 'ML-DSA-87':
      return require('internal/crypto/ml_dsa').mlDsaGenerateKey(normalizedAlgorithm, extractable, usages);
    case 'ML-KEM-512':
    // Fall through
    case 'ML-KEM-768':
    // Fall through
    case 'ML-KEM-1024':
      return require('internal/crypto/ml_kem').mlKemGenerateKey(normalizedAlgorithm, extractable, usages);
    case 'KMAC128':
    // Fall through
    case 'KMAC256':
      return require('internal/crypto/mac').kmacGenerateKey(normalizedAlgorithm, extractable, usages);
    default:
      throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
  }
}
function deriveBits(algorithm, baseKey) {
...
function deriveBitsImpl(algorithm, baseKey) {
  var length = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'deriveBits' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 2, {
    prefix
  });
  algorithm = webidl.converters.AlgorithmIdentifier(algorithm, {
  ...
  var normalizedAlgorithm = normalizeAlgorithm(algorithm, 'deriveBits');
  if (!hasCryptoKeyUsage(baseKey, 'deriveBits')) {
    throw lazyDOMException('baseKey does not have deriveBits usage', 'InvalidAccessError');
  }
  if (getCryptoKeyAlgorithm(baseKey).name !== normalizedAlgorithm.name) throw lazyDOMException('Key algorithm mismatch', 'InvalidAccessError');
  switch (normalizedAlgorithm.name) {
    case 'X25519':
    // Fall through
    case 'X448':
    // Fall through
    case 'ECDH':
      return require('internal/crypto/diffiehellman').ecdhDeriveBits(normalizedAlgorithm, baseKey, length);
    case 'HKDF':
      return require('internal/crypto/hkdf').hkdfDeriveBits(normalizedAlgorithm, baseKey, length);
    case 'PBKDF2':
      return require('internal/crypto/pbkdf2').pbkdf2DeriveBits(normalizedAlgorithm, baseKey, length);
    case 'Argon2d':
    // Fall through
    case 'Argon2i':
    // Fall through
    case 'Argon2id':
      return require('internal/crypto/argon2').argon2DeriveBits(normalizedAlgorithm, baseKey, length);
  }
  throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
}
function getKeyLength(_ref) {
  var {
    name,
    length,
    hash
  ...
  switch (name) {
    ...
    case 'AES-OCB':
    case 'AES-KW':
      if (length !== 128 && length !== 192 && length !== 256) throw lazyDOMException('Invalid key length', 'OperationError');
      return length;
    case 'HMAC':
      if (length === undefined) {
...
function deriveKeyImpl(algorithm, baseKey, derivedKeyType, extractable, keyUsages) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'deriveKey' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 5, {
    prefix
  });
  algorithm = webidl.converters.AlgorithmIdentifier(algorithm, {
    prefix,
  ...
  var normalizedAlgorithm = normalizeAlgorithm(algorithm, 'deriveBits');
  var normalizedDerivedKeyAlgorithmImport = normalizeAlgorithm(derivedKeyType, 'importKey');
  var normalizedDerivedKeyAlgorithmLength = normalizeAlgorithm(derivedKeyType, 'get key length');
  if (!hasCryptoKeyUsage(baseKey, 'deriveKey')) {
    throw lazyDOMException('baseKey does not have deriveKey usage', 'InvalidAccessError');
  }
  if (getCryptoKeyAlgorithm(baseKey).name !== normalizedAlgorithm.name) throw lazyDOMException('Key algorithm mismatch', 'InvalidAccessError');
  var length = getKeyLength(normalizedDerivedKeyAlgorithmLength);
  var secret;
  switch (normalizedAlgorithm.name) {
    case 'X25519':
    // Fall through
    ...
    // Fall through
    case 'ECDH':
      secret = require('internal/crypto/diffiehellman').ecdhDeriveBits(normalizedAlgorithm, baseKey, length);
      break;
    case 'HKDF':
      secret = require('internal/crypto/hkdf').hkdfDeriveBits(normalizedAlgorithm, baseKey, length);
      break;
    case 'PBKDF2':
      secret = require('internal/crypto/pbkdf2').pbkdf2DeriveBits(normalizedAlgorithm, baseKey, length);
      break;
    case 'Argon2d':
    // Fall through
    case 'Argon2i':
    // Fall through
    case 'Argon2id':
      secret = require('internal/crypto/argon2').argon2DeriveBits(normalizedAlgorithm, baseKey, length);
      break;
    default:
      throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
  }
  return jobPromiseThen(secret, secret => FunctionPrototypeCall(importKeySync, this, 'raw-secret', secret, normalizedDerivedKeyAlgorithmImport, extractable, usages));
}
function exportKeySpki(key) {
  switch (getCryptoKeyAlgorithm(key).name) {
    ...
    // Fall through
    case 'RSA-OAEP':
      return require('internal/crypto/rsa').rsaExportKey(key, kWebCryptoKeyFormatSPKI);
    case 'ECDSA':
    // Fall through
    case 'ECDH':
      return require('internal/crypto/ec').ecExportKey(key, kWebCryptoKeyFormatSPKI);
    case 'Ed25519':
    // Fall through
    case 'Ed448':
    // Fall through
    ...
    // Fall through
    case 'X448':
      return require('internal/crypto/cfrg').cfrgExportKey(key, kWebCryptoKeyFormatSPKI);
    case 'ML-DSA-44':
    // Fall through
    case 'ML-DSA-65':
    // Fall through
    case 'ML-DSA-87':
      return require('internal/crypto/ml_dsa').mlDsaExportKey(key, kWebCryptoKeyFormatSPKI);
    case 'ML-KEM-512':
    // Fall through
    case 'ML-KEM-768':
    // Fall through
    case 'ML-KEM-1024':
      return require('internal/crypto/ml_kem').mlKemExportKey(key, kWebCryptoKeyFormatSPKI);
    default:
      return undefined;
  }
}
function exportKeyPkcs8(key) {
  switch (getCryptoKeyAlgorithm(key).name) {
    ...
    // Fall through
    case 'RSA-OAEP':
      return require('internal/crypto/rsa').rsaExportKey(key, kWebCryptoKeyFormatPKCS8);
    case 'ECDSA':
    // Fall through
    case 'ECDH':
      return require('internal/crypto/ec').ecExportKey(key, kWebCryptoKeyFormatPKCS8);
    case 'Ed25519':
    // Fall through
    case 'Ed448':
    // Fall through
    ...
    // Fall through
    case 'X448':
      return require('internal/crypto/cfrg').cfrgExportKey(key, kWebCryptoKeyFormatPKCS8);
    case 'ML-DSA-44':
    // Fall through
    case 'ML-DSA-65':
    // Fall through
    case 'ML-DSA-87':
      return require('internal/crypto/ml_dsa').mlDsaExportKey(key, kWebCryptoKeyFormatPKCS8);
    case 'ML-KEM-512':
    // Fall through
    case 'ML-KEM-768':
    // Fall through
    case 'ML-KEM-1024':
      return require('internal/crypto/ml_kem').mlKemExportKey(key, kWebCryptoKeyFormatPKCS8);
    default:
      return undefined;
  }
}
function exportKeyRawPublic(key, format) {
  switch (getCryptoKeyAlgorithm(key).name) {
    ...
    // Fall through
    case 'ECDH':
      return require('internal/crypto/ec').ecExportKey(key, kWebCryptoKeyFormatRaw);
    case 'Ed25519':
    // Fall through
    case 'Ed448':
    // Fall through
    ...
    // Fall through
    case 'X448':
      return require('internal/crypto/cfrg').cfrgExportKey(key, kWebCryptoKeyFormatRaw);
    case 'ML-DSA-44':
    // Fall through
    case 'ML-DSA-65':
    // Fall through
    case 'ML-DSA-87':
      {
        ...
        return require('internal/crypto/ml_dsa').mlDsaExportKey(key, kWebCryptoKeyFormatRaw);
      }
    case 'ML-KEM-512':
    // Fall through
    case 'ML-KEM-768':
    ...
    case 'ML-KEM-1024':
      {
        ...
        return require('internal/crypto/ml_kem').mlKemExportKey(key, kWebCryptoKeyFormatRaw);
      }
    default:
      return undefined;
  }
...
function exportKeyJWK(key) {
  var algorithm = getCryptoKeyAlgorithm(key);
  var alg;
  switch (algorithm.name) {
    case 'RSASSA-PKCS1-v1_5':
      {
        alg = normalizeHashName(algorithm.hash.name, normalizeHashName.kContextJwkRsa);
        break;
      }
    case 'RSA-PSS':
      {
        alg = normalizeHashName(algorithm.hash.name, normalizeHashName.kContextJwkRsaPss);
        break;
      }
    case 'RSA-OAEP':
      {
        alg = normalizeHashName(algorithm.hash.name, normalizeHashName.kContextJwkRsaOaep);
        break;
      }
    case 'ECDSA':
    // Fall through
    ...
    // Fall through
    case 'AES-KW':
      alg = require('internal/crypto/aes').getAlgorithmName(algorithm.name, algorithm.length);
      break;
    case 'ChaCha20-Poly1305':
      alg = 'C20P';
      break;
    case 'HMAC':
      {
        alg = normalizeHashName(algorithm.hash.name, normalizeHashName.kContextJwkHmac);
        break;
      }
    case 'KMAC128':
      alg = 'K128';
...
function exportKeyImpl(format, key) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'exportKey' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 2, {
    prefix
  });
...
function detachFromUserPrototypes(value) {
  ...
  if (ArrayIsArray(value)) {
    ...
    for (var n = 0; n < value.length; n++) detachFromUserPrototypes(value[n]);
    return;
  }
  var keys = ObjectKeys(value);
  for (var _n = 0; _n < keys.length; _n++) detachFromUserPrototypes(value[keys[_n]]);
}
...
function parseJwk(data) {
  ...
  try {
    ...
    // spec-required UTF-8.
    var json = decodeUTF8(data, false, true);
    var result = JSONParse(json);
    detachFromUserPrototypes(result);
    key = webidl.converters.JsonWebKey(result);
  } catch (err) {
    throw lazyDOMException('Invalid wrapped JWK key', {
      name: 'DataError',
      cause: err
    });
  }
...
function importKeySync(format, keyData, algorithm, extractable, usages) {
  ...
  switch (algorithm.name) {
    ...
    case 'RSA-OAEP':
      ...
      result = require('internal/crypto/rsa').rsaImportKey(format, keyData, algorithm, extractable, usages);
      break;
    case 'ECDSA':
    // Fall through
    case 'ECDH':
      format = aliasKeyFormat(format);
      result = require('internal/crypto/ec').ecImportKey(format, keyData, algorithm, extractable, usages);
      break;
    case 'Ed25519':
    // Fall through
    case 'Ed448':
    ...
    case 'X448':
      format = aliasKeyFormat(format);
      result = require('internal/crypto/cfrg').cfrgImportKey(format, keyData, algorithm, extractable, usages);
      break;
    case 'HMAC':
    // Fall through
    case 'KMAC128':
    // Fall through
    case 'KMAC256':
      result = require('internal/crypto/mac').macImportKey(format, keyData, algorithm, extractable, usages);
      break;
    case 'AES-CTR':
    // Fall through
    case 'AES-CBC':
    ...
    // Fall through
    case 'AES-OCB':
      result = require('internal/crypto/aes').aesImportKey(algorithm, format, keyData, extractable, usages);
      break;
    case 'ChaCha20-Poly1305':
      result = require('internal/crypto/chacha20_poly1305').c20pImportKey(algorithm, format, keyData, extractable, usages);
      break;
    case 'HKDF':
    // Fall through
    case 'PBKDF2':
      format = aliasKeyFormat(format);
      result = importGenericSecretKey(algorithm, format, keyData, extractable, usages);
      break;
    case 'Argon2d':
    // Fall through
    case 'Argon2i':
    ...
    // Fall through
    case 'ML-DSA-87':
      result = require('internal/crypto/ml_dsa').mlDsaImportKey(format, keyData, algorithm, extractable, usages);
      break;
    case 'ML-KEM-512':
    // Fall through
    case 'ML-KEM-768':
    // Fall through
    case 'ML-KEM-1024':
      result = require('internal/crypto/ml_kem').mlKemImportKey(format, keyData, algorithm, extractable, usages);
      break;
  }
  if (!result) {
    throw lazyDOMException(`Unable to import ${algorithm.name} using ${format} format`, 'NotSupportedError');
  }
  var type = getCryptoKeyType(result);
...
function importKeyImpl(format, keyData, algorithm, extractable, keyUsages) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'importKey' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 5, {
    prefix
  });
  format = webidl.converters.KeyFormat(format, {
  ...
  var type = format === 'jwk' ? 'JsonWebKey' : 'BufferSource';
  ...
  var normalizedAlgorithm = normalizeAlgorithm(algorithm, 'importKey');
  return FunctionPrototypeCall(importKeySync, this, format, keyData, normalizedAlgorithm, extractable, usages);
}
...
function wrapKeyImpl(format, key, wrappingKey, wrapAlgorithm) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'wrapKey' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 4, {
    prefix
  });
  format = webidl.converters.KeyFormat(format, {
    prefix,
  ...
  var algorithm = webidl.converters.AlgorithmIdentifier(wrapAlgorithm, {
    prefix,
    context: '4th argument'
  });
  var normalizedAlgorithm;
  try {
    normalizedAlgorithm = normalizeAlgorithm(algorithm, 'wrapKey');
  } catch {
    normalizedAlgorithm = normalizeAlgorithm(algorithm, 'encrypt');
  }
  if (normalizedAlgorithm.name !== getCryptoKeyAlgorithm(wrappingKey).name) throw lazyDOMException('Key algorithm mismatch', 'InvalidAccessError');
  if (!hasCryptoKeyUsage(wrappingKey, 'wrapKey')) throw lazyDOMException('Unable to use this key to wrapKey', 'InvalidAccessError');
  var exportedKey = exportKeySync(format, key);
  var bytes = exportedKey;
  if (format === 'jwk') {
    ...
    var json = JSONStringify(exportedKey);
    ...
    if (normalizedAlgorithm.name === 'AES-KW' && json.length % 8 !== 0) {
      bytes = encodeUtf8String(json + StringPrototypeRepeat(' ', 8 - json.length % 8));
    } else {
      bytes = encodeUtf8String(json);
    }
  }
  return cipherOrWrap(kWebCryptoCipherEncrypt, normalizedAlgorithm, wrappingKey, bytes, 'wrapKey');
}
...
function unwrapKeyImpl(format, wrappedKey, unwrappingKey, unwrapAlgorithm, unwrappedKeyAlgorithm, extractable, keyUsages) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'unwrapKey' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 7, {
    prefix
  });
  format = webidl.converters.KeyFormat(format, {
    prefix,
  ...
  var algorithm = webidl.converters.AlgorithmIdentifier(unwrapAlgorithm, {
    prefix,
    context: '4th argument'
  });
  unwrappedKeyAlgorithm = webidl.converters.AlgorithmIdentifier(unwrappedKeyAlgorithm, {
    prefix,
    context: '5th argument'
  });
  extractable = webidl.converters.boolean(extractable, {
  ...
  var normalizedKeyAlgorithm = normalizeAlgorithm(unwrappedKeyAlgorithm, 'importKey');
  if (normalizedAlgorithm.name !== getCryptoKeyAlgorithm(unwrappingKey).name) throw lazyDOMException('Key algorithm mismatch', 'InvalidAccessError');
  ...
  var bytes = cipherOrWrap(kWebCryptoCipherDecrypt, normalizedAlgorithm, unwrappingKey, wrappedKey, 'unwrapKey');
  return jobPromiseThen(bytes, bytes => {
    var keyData = bytes;
    if (format === 'jwk') {
      keyData = parseJwk(bytes);
    }
    return FunctionPrototypeCall(importKeySync, this, format, keyData, normalizedKeyAlgorithm, extractable, usages);
  });
}
function signVerify(algorithm, key, data, signature) {
  var operation = signature !== undefined ? 'verify' : 'sign'; // This is also usage
  var normalizedAlgorithm = normalizeAlgorithm(algorithm, operation);
  if (normalizedAlgorithm.name !== getCryptoKeyAlgorithm(key).name) throw lazyDOMException('Key algorithm mismatch', 'InvalidAccessError');
  if (!hasCryptoKeyUsage(key, operation)) throw lazyDOMException(`Unable to use this key to ${operation}`, 'InvalidAccessError');
  switch (normalizedAlgorithm.name) {
    case 'RSA-PSS':
    // Fall through
    case 'RSASSA-PKCS1-v1_5':
      return require('internal/crypto/rsa').rsaSignVerify(key, data, normalizedAlgorithm, signature);
    case 'ECDSA':
      return require('internal/crypto/ec').ecdsaSignVerify(key, data, normalizedAlgorithm, signature);
    case 'Ed25519':
    // Fall through
    case 'Ed448':
      // Fall through
      return require('internal/crypto/cfrg').eddsaSignVerify(key, data, normalizedAlgorithm, signature);
    case 'HMAC':
      return require('internal/crypto/mac').hmacSignVerify(key, data, normalizedAlgorithm, signature);
    case 'ML-DSA-44':
    // Fall through
    case 'ML-DSA-65':
    // Fall through
    case 'ML-DSA-87':
      return require('internal/crypto/ml_dsa').mlDsaSignVerify(key, data, normalizedAlgorithm, signature);
    case 'KMAC128':
    // Fall through
    case 'KMAC256':
      return require('internal/crypto/mac').kmacSignVerify(key, data, normalizedAlgorithm, signature);
  }
  throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
}
...
function signImpl(algorithm, key, data) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'sign' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 3, {
    prefix
  });
  algorithm = webidl.converters.AlgorithmIdentifier(algorithm, {
...
function verifyImpl(algorithm, key, signature, data) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'verify' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 4, {
    prefix
  });
  algorithm = webidl.converters.AlgorithmIdentifier(algorithm, {
...
function cipherOrWrap(mode, normalizedAlgorithm, key, data, operation) {
  ...
  switch (normalizedAlgorithm.name) {
    case 'RSA-OAEP':
      return require('internal/crypto/rsa').rsaCipher(mode, key, data, normalizedAlgorithm);
    case 'AES-CTR':
    // Fall through
    case 'AES-CBC':
    // Fall through
    ...
    // Fall through
    case 'AES-OCB':
      return require('internal/crypto/aes').aesCipher(mode, key, data, normalizedAlgorithm);
    case 'ChaCha20-Poly1305':
      return require('internal/crypto/chacha20_poly1305').c20pCipher(mode, key, data, normalizedAlgorithm);
    case 'AES-KW':
...
function encryptImpl(algorithm, key, data) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'encrypt' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 3, {
    prefix
  });
  algorithm = webidl.converters.AlgorithmIdentifier(algorithm, {
  ...
  var normalizedAlgorithm = normalizeAlgorithm(algorithm, 'encrypt');
  if (normalizedAlgorithm.name !== getCryptoKeyAlgorithm(key).name) throw lazyDOMException('Key algorithm mismatch', 'InvalidAccessError');
  if (!hasCryptoKeyUsage(key, 'encrypt')) throw lazyDOMException('Unable to use this key to encrypt', 'InvalidAccessError');
  return cipherOrWrap(kWebCryptoCipherEncrypt, normalizedAlgorithm, key, data, 'encrypt');
}
...
function decryptImpl(algorithm, key, data) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'decrypt' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 3, {
    prefix
  });
  algorithm = webidl.converters.AlgorithmIdentifier(algorithm, {
  ...
  var normalizedAlgorithm = normalizeAlgorithm(algorithm, 'decrypt');
  if (normalizedAlgorithm.name !== getCryptoKeyAlgorithm(key).name) throw lazyDOMException('Key algorithm mismatch', 'InvalidAccessError');
  if (!hasCryptoKeyUsage(key, 'decrypt')) throw lazyDOMException('Unable to use this key to decrypt', 'InvalidAccessError');
  return cipherOrWrap(kWebCryptoCipherDecrypt, normalizedAlgorithm, key, data, 'decrypt');
}
...
function getPublicKeyImpl(key, keyUsages) {
  ...
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'getPublicKey' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 2, {
    prefix
  });
  key = webidl.converters.CryptoKey(key, {
  ...
  var type = getCryptoKeyType(key);
  if (type !== 'private') throw lazyDOMException('key must be a private key', type === 'secret' ? 'NotSupportedError' : 'InvalidAccessError');

  // TODO(panva): this is by no means a hot path, but let's still follow up to get
  //              rid of this awkwardness
  var keyObject = createPublicKey(new PrivateKeyObject(getCryptoKeyHandle(key)));
...
function encapsulateBitsImpl(encapsulationAlgorithm, encapsulationKey) {
  ...
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'encapsulateBits' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 2, {
    prefix
  });
  ...
  var normalizedEncapsulationAlgorithm = normalizeAlgorithm(encapsulationAlgorithm, 'encapsulate');
  var keyAlgorithm = getCryptoKeyAlgorithm(encapsulationKey);
  ...
  if (!hasCryptoKeyUsage(encapsulationKey, 'encapsulateBits')) {
    throw lazyDOMException('encapsulationKey does not have encapsulateBits usage', 'InvalidAccessError');
  }
  switch (keyAlgorithm.name) {
    case 'ML-KEM-512':
    case 'ML-KEM-768':
    case 'ML-KEM-1024':
      return require('internal/crypto/ml_kem').mlKemEncapsulate(encapsulationKey);
  }
  throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
}
...
function encapsulateKeyImpl(encapsulationAlgorithm, encapsulationKey, sharedKeyAlgorithm, extractable, keyUsages) {
  emitExperimentalWarning('The encapsulateKey Web Crypto API method');
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'encapsulateKey' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 5, {
    prefix
  });
  encapsulationAlgorithm = webidl.converters.AlgorithmIdentifier(encapsulationAlgorithm, {
  ...
  var normalizedEncapsulationAlgorithm = normalizeAlgorithm(encapsulationAlgorithm, 'encapsulate');
  var normalizedSharedKeyAlgorithm = normalizeAlgorithm(sharedKeyAlgorithm, 'importKey');
  var keyAlgorithm = getCryptoKeyAlgorithm(encapsulationKey);
  if (normalizedEncapsulationAlgorithm.name !== keyAlgorithm.name) {
    throw lazyDOMException('key algorithm mismatch', 'InvalidAccessError');
  }
  ...
  var encapsulatedBits;
  switch (keyAlgorithm.name) {
    case 'ML-KEM-512':
    case 'ML-KEM-768':
    case 'ML-KEM-1024':
      encapsulatedBits = require('internal/crypto/ml_kem').mlKemEncapsulate(encapsulationKey);
      break;
    default:
      throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
  }
  return jobPromiseThen(encapsulatedBits, encapsulatedBits => {
    var sharedKey = FunctionPrototypeCall(importKeySync, this, 'raw-secret', encapsulatedBits.sharedKey, normalizedSharedKeyAlgorithm, extractable, usages);
    return {
...
function decapsulateBitsImpl(decapsulationAlgorithm, decapsulationKey, ciphertext) {
  ...
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'decapsulateBits' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 3, {
    prefix
  });
  ...
  var normalizedDecapsulationAlgorithm = normalizeAlgorithm(decapsulationAlgorithm, 'decapsulate');
  var keyAlgorithm = getCryptoKeyAlgorithm(decapsulationKey);
  if (normalizedDecapsulationAlgorithm.name !== keyAlgorithm.name) {
    throw lazyDOMException('key algorithm mismatch', 'InvalidAccessError');
  }
  if (!hasCryptoKeyUsage(decapsulationKey, 'decapsulateBits')) {
    throw lazyDOMException('decapsulationKey does not have decapsulateBits usage', 'InvalidAccessError');
  }
  switch (keyAlgorithm.name) {
    case 'ML-KEM-512':
    case 'ML-KEM-768':
    case 'ML-KEM-1024':
      return require('internal/crypto/ml_kem').mlKemDecapsulate(decapsulationKey, ciphertext);
  }
  throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
}
function decapsulateKey(decapsulationAlgorithm, decapsulationKey, ciphertext, sharedKeyAlgorithm, extractable, keyUsages) {
  return callSubtleCryptoMethod(decapsulateKeyImpl, this, arguments);
}
function decapsulateKeyImpl(decapsulationAlgorithm, decapsulationKey, ciphertext, sharedKeyAlgorithm, extractable, keyUsages) {
  emitExperimentalWarning('The decapsulateKey Web Crypto API method');
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'decapsulateKey' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 6, {
    prefix
  });
  decapsulationAlgorithm = webidl.converters.AlgorithmIdentifier(decapsulationAlgorithm, {
    prefix,
  ...
  if (!hasCryptoKeyUsage(decapsulationKey, 'decapsulateKey')) {
    throw lazyDOMException('decapsulationKey does not have decapsulateKey usage', 'InvalidAccessError');
  }
  var decapsulatedBits;
  switch (keyAlgorithm.name) {
    case 'ML-KEM-512':
    case 'ML-KEM-768':
    case 'ML-KEM-1024':
      decapsulatedBits = require('internal/crypto/ml_kem').mlKemDecapsulate(decapsulationKey, ciphertext);
      break;
    default:
      throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
  }
  return jobPromiseThen(decapsulatedBits, decapsulatedBits => FunctionPrototypeCall(importKeySync, this, 'raw-secret', decapsulatedBits, normalizedSharedKeyAlgorithm, extractable, usages));
}
...
var SubtleCrypto = /*#__PURE__*/function () {
  ...
  return _createClass(SubtleCrypto, null, [{
    key: "supports",
    value: function supports(operation, algorithm) {
      var lengthOrAdditionalAlgorithm = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
      ...
      webidl ??= require('internal/crypto/webidl');
      var prefix = "Failed to execute 'supports' on 'SubtleCrypto'";
      webidl.requiredArguments(arguments.length, 2, {
        prefix
      });
      ...
      if (operation === 'deriveKey') {
        ...
        var normalizedAdditionalAlgorithm;
        try {
...
function check(op, alg, length) {
  ...
  switch (op) {
    ...
    case 'deriveBits':
      {
        ...
        if (normalizedAlgorithm.name === 'ECDH') {
          var namedCurve = getCryptoKeyAlgorithm(normalizedAlgorithm.public).namedCurve;
          var maxLength = {
            '__proto__': null,
            'P-256': 256,
            'P-384': 384,
    ...
    case 'generateKey':
      {
        if (normalizedAlgorithm.name === 'HMAC' && normalizedAlgorithm.length === undefined && StringPrototypeStartsWith(normalizedAlgorithm.hash.name, 'SHA3-')) {
          return false;
        }
        return true;
      }
...
var subtle = ReflectConstruct(function () {}, [], SubtleCrypto);
...
var crypto = ReflectConstruct(function () {}, [], Crypto);
function getRandomValues(array) {
  if (this !== crypto) throw new ERR_INVALID_THIS('Crypto');
  webidl ??= require('internal/crypto/webidl');
  var prefix = "Failed to execute 'getRandomValues' on 'Crypto'";
