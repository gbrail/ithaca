...
var kState = Symbol('kState');
var kFinalized = Symbol('kFinalized');
...
var maybeEmitDeprecationWarning = getDeprecationWarningEmitter('DEP0198', 'Creating SHAKE128/256 digests without an explicit options.outputLength is deprecated.', undefined, false, algorithm => {
...
function Hash(algorithm, options) {
  ...
  var algorithmId = isCopy ? -1 : getCachedHashId(algorithm);
...
Hash.prototype.update = function update(data, encoding) {
  ...
  if (!this[kHandle].update(data, encoding)) throw new ERR_CRYPTO_HASH_UPDATE_FAILED();
  return this;
};
Hash.prototype.digest = function digest(outputEncoding) {
  ...
  var ret = this[kHandle].digest(outputEncoding && `${outputEncoding}`);
...
Hmac.prototype.digest = function digest(outputEncoding) {
  ...
  if (state[kFinalized]) {
    var buf = Buffer.from('');
    if (outputEncoding && outputEncoding !== 'buffer') return buf.toString(outputEncoding);
    return buf;
  }
...
function asyncDigest(algorithm, data) {
  ...
  switch (algorithm.name) {
    ...
    // Fall through
    case 'cSHAKE256':
      return jobPromise(() => new HashJob(kCryptoJobWebCrypto, normalizeHashName(algorithm.name), data, algorithm.outputLength));
    case 'TurboSHAKE128':
    // Fall through
    case 'TurboSHAKE256':
      return jobPromise(() => new TurboShakeJob(kCryptoJobWebCrypto, algorithm.name, algorithm.domainSeparation ?? 0x1f, algorithm.outputLength / 8, data));
    case 'KT128':
    // Fall through
    case 'KT256':
      return jobPromise(() => new KangarooTwelveJob(kCryptoJobWebCrypto, algorithm.name, algorithm.customization, algorithm.outputLength / 8, data));
  }
...
function hash(algorithm, input, options) {
  ...
  return oneShotDigest(algorithm, getCachedHashId(algorithm), getHashCache(), input, normalized, encodingsMap[normalized], outputLength);
}
module.exports = {
  Hash,
  Hmac,
