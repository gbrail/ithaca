...
var validateParameters = hideStackFrames((hash, key, salt, info, length) => {
...
function prepareKey(key) {
  if (isKeyObject(key)) return prepareSecretKey(key);
  if (isAnyArrayBuffer(key)) return key;
  key = toBuf(key);
  if (!isArrayBufferView(key)) {
    throw new ERR_INVALID_ARG_TYPE('ikm', ['string', 'SecretKeyObject', 'ArrayBuffer', 'TypedArray', 'DataView', 'Buffer'], key);
  }
  return key;
}
function hkdf(hash, key, salt, info, length, callback) {
  ...
  var job = new HKDFJob(kCryptoJobAsync, hash, key, salt, info, length);
...
function hkdfSync(hash, key, salt, info, length) {
  ...
  var job = new HKDFJob(kCryptoJobSync, hash, key, salt, info, length);
  var {
    0: err,
    1: bits
  } = job.run();
...
function hkdfDeriveBits(algorithm, baseKey, length) {
  ...
  return jobPromise(() => new HKDFJob(kCryptoJobWebCrypto, normalizeHashName(hash.name), getCryptoKeyHandle(baseKey), salt, info, length / 8));
}
module.exports = {
  hkdf,
  hkdfSync,
