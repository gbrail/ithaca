...
function rsaOaepCipher(mode, key, data, algorithm) {
  ...
  var type = mode === kWebCryptoCipherEncrypt ? 'public' : 'private';
  ...
  return jobPromise(() => new RSACipherJob(kCryptoJobWebCrypto, mode, getCryptoKeyHandle(key), data, kKeyVariantRSA_OAEP, normalizeHashName(getCryptoKeyAlgorithm(key).hash.name), algorithm.label));
}
function rsaKeyGenerate(algorithm, extractable, usages) {
  var publicExponentConverted = bigIntArrayToUnsignedInt(algorithm.publicExponent);
  ...
  return jobPromise(() => new RsaKeyPairGenJob(kCryptoJobWebCrypto, kKeyVariantRSA_SSA_PKCS1_v1_5, modulusLength, publicExponentConverted, keyAlgorithm, getUsagesMask(publicUsages), getUsagesMask(privateUsages), extractable));
}
  ...
  try {
    switch (format) {
      case kWebCryptoKeyFormatSPKI:
        {
          return TypedArrayPrototypeGetBuffer(getCryptoKeyHandle(key).export(kKeyFormatDER, kWebCryptoKeyFormatSPKI));
        }
      case kWebCryptoKeyFormatPKCS8:
        {
          return TypedArrayPrototypeGetBuffer(getCryptoKeyHandle(key).export(kKeyFormatDER, kWebCryptoKeyFormatPKCS8, null, null));
        }
      default:
        return undefined;
    }
...
function rsaImportKey(format, keyData, algorithm, extractable, usages) {
  var usagesSet = new SafeSet(usages);
  var handle;
  switch (format) {
    case 'KeyObject':
      {
        verifyAcceptableRsaKeyUse(algorithm.name, getKeyObjectType(keyData) === 'public', usagesSet);
    ...
    case 'jwk':
      {
        var expectedUse = algorithm.name === 'RSA-OAEP' ? 'enc' : 'sig';
        validateJwk(keyData, 'RSA', extractable, usagesSet, expectedUse);
        if (keyData.alg !== undefined) {
          var expected = normalizeHashName(algorithm.hash.name, algorithm.name === 'RSASSA-PKCS1-v1_5' ? normalizeHashName.kContextJwkRsa : algorithm.name === 'RSA-PSS' ? normalizeHashName.kContextJwkRsaPss : normalizeHashName.kContextJwkRsaOaep);
...
function rsaSignVerify(key, data, _ref, signature) {
  var {
    saltLength
  } = _ref;
  ...
  if (algorithm.name === 'RSA-PSS') {
    try {
      validateInt32(saltLength, 'algorithm.saltLength', 0, MathCeil((algorithm.modulusLength - 1) / 8) - getDigestSizeInBytes(algorithm.hash.name) - 2);
    } catch (err) {
      throw lazyDOMException('The operation failed for an operation-specific reason', {
        name: 'OperationError',
        cause: err
  ...
  return jobPromise(() => new SignJob(kCryptoJobWebCrypto, signature === undefined ? kSignJobModeSign : kSignJobModeVerify, getCryptoKeyHandle(key), undefined, undefined, undefined, undefined, data, normalizeHashName(algorithm.hash.name), saltLength, algorithm.name === 'RSA-PSS' ? RSA_PKCS1_PSS_PADDING : undefined, undefined, undefined, signature));
}
module.exports = {
