...
function getVariant(name, length) {
  switch (name) {
    ...
    case 'AES-OCB':
      switch (length) {
        case 128:
          return kKeyVariantAES_OCB_128;
        case 192:
          return kKeyVariantAES_OCB_192;
        case 256:
          return kKeyVariantAES_OCB_256;
      }
      break;
  }
}
function asyncAesCtrCipher(mode, key, data, algorithm) {
  return jobPromise(() => new AESCipherJob(kCryptoJobWebCrypto, mode, getCryptoKeyHandle(key), data, getVariant('AES-CTR', getCryptoKeyAlgorithm(key).length), algorithm.counter, algorithm.length));
}
function asyncAesCbcCipher(mode, key, data, algorithm) {
  return jobPromise(() => new AESCipherJob(kCryptoJobWebCrypto, mode, getCryptoKeyHandle(key), data, getVariant('AES-CBC', getCryptoKeyAlgorithm(key).length), algorithm.iv));
}
function asyncAesKwCipher(mode, key, data) {
  return jobPromise(() => new AESCipherJob(kCryptoJobWebCrypto, mode, getCryptoKeyHandle(key), data, getVariant('AES-KW', getCryptoKeyAlgorithm(key).length)));
}
function asyncAesGcmCipher(mode, key, data, algorithm) {
  var {
    tagLength = 128
  } = algorithm;
  var tagByteLength = tagLength / 8;
  return jobPromise(() => new AESCipherJob(kCryptoJobWebCrypto, mode, getCryptoKeyHandle(key), data, getVariant('AES-GCM', getCryptoKeyAlgorithm(key).length), algorithm.iv, tagByteLength, algorithm.additionalData));
}
function asyncAesOcbCipher(mode, key, data, algorithm) {
  ...
  var tagByteLength = tagLength / 8;
  return jobPromise(() => new AESCipherJob(kCryptoJobWebCrypto, mode, getCryptoKeyHandle(key), data, getVariant('AES-OCB', getCryptoKeyAlgorithm(key).length), algorithm.iv, tagByteLength, algorithm.additionalData));
}
function aesCipher(mode, key, data, algorithm) {
  switch (algorithm.name) {
    ...
    case 'AES-OCB':
      return asyncAesOcbCipher(mode, key, data, algorithm);
    case 'AES-KW':
...
function aesGenerateKey(algorithm, extractable, usages) {
  ...
  var checkUsages = ['wrapKey', 'unwrapKey'];
...
function aesImportKey(algorithm, format, keyData, extractable, usages) {
  var {
    name
  } = algorithm;
  var checkUsages = ['wrapKey', 'unwrapKey'];
    ...
    case 'jwk':
      {
        ...
        if (keyData.alg !== undefined) {
          if (keyData.alg !== getAlgorithmName(algorithm.name, length)) throw lazyDOMException('JWK "alg" does not match the requested algorithm', 'DataError');
        }
        break;
      }
    default:
