...
var normalizeHashName = require('internal/crypto/hashnames');
var {
  codes: {
...
function getCachedHashId(algorithm) {
  var result = getHashCache()[algorithm];
  return result === undefined ? -1 : result;
}
var getCiphers = cachedResult(() => filterDuplicateStrings(_getCiphers()));
var getHashes = cachedResult(() => filterDuplicateStrings(_getHashes()));
var getCurves = cachedResult(() => filterDuplicateStrings(_getCurves()));
function setEngine(id, flags) {
  ...
  if (typeof _setEngine !== 'function') throw new ERR_CRYPTO_CUSTOM_ENGINE_NOT_SUPPORTED();
  if (!_setEngine(id, flags)) throw new ERR_CRYPTO_ENGINE_UNKNOWN(id);
}
var getArrayBufferOrView = hideStackFrames((buffer, name, encoding) => {
  ...
  if (!isArrayBufferView(buffer)) {
    throw new ERR_INVALID_ARG_TYPE.HideStackFramesError(name, ['string', 'ArrayBuffer', 'Buffer', 'TypedArray', 'DataView'], buffer);
  }
  return buffer;
});

// The maximum buffer size that we'll support in the WebCrypto impl
var kMaxBufferLength = 2 ** 31 - 1;

// The EC named curves that we currently support via the Web Crypto API.
var kNamedCurveAliases = {
  'P-256': 'prime256v1',
  'P-384': 'secp384r1',
  'P-521': 'secp521r1'
};
...
var conditionalAlgorithms = {
  'AES-OCB': !!hasAesOcbMode,
  'Argon2d': !!Argon2Job,
  'Argon2i': !!Argon2Job,
  'Argon2id': !!Argon2Job,
  'ChaCha20-Poly1305': process.features.openssl_is_boringssl || ArrayPrototypeIncludes(getCiphers(), 'chacha20-poly1305'),
  'cSHAKE128': !process.features.openssl_is_boringssl || ArrayPrototypeIncludes(getHashes(), 'shake128'),
  'cSHAKE256': !process.features.openssl_is_boringssl || ArrayPrototypeIncludes(getHashes(), 'shake256'),
  ...
  'SHA3-256': !process.features.openssl_is_boringssl || ArrayPrototypeIncludes(getHashes(), 'sha3-256'),
  'SHA3-384': !process.features.openssl_is_boringssl || ArrayPrototypeIncludes(getHashes(), 'sha3-384'),
  'SHA3-512': !process.features.openssl_is_boringssl || ArrayPrototypeIncludes(getHashes(), 'sha3-512'),
  'X448': !process.features.openssl_is_boringssl
};

// Experimental algorithms
var experimentalAlgorithms = ['AES-OCB', 'Argon2d', 'Argon2i', 'Argon2id', 'ChaCha20-Poly1305', 'cSHAKE128', 'cSHAKE256', 'Ed448', 'KMAC128', 'KMAC256', 'ML-DSA-44', 'ML-DSA-65', 'ML-DSA-87', 'ML-KEM-512', 'ML-KEM-768', 'ML-KEM-1024', 'SHA3-256', 'SHA3-384', 'SHA3-512', 'TurboSHAKE128', 'TurboSHAKE256', 'KT128', 'KT256', 'X448'];
...
function createSupportedAlgorithms(algorithmDefs) {
  var result = {};
  var nameMap = {};
  var _loop = function (algorithmName) {
    // Skip algorithms that are conditionally not supported
    if (ObjectPrototypeHasOwnProperty(conditionalAlgorithms, algorithmName) && !conditionalAlgorithms[algorithmName]) {
      return 1; // continue
    }
    var _loop2 = function (dict) {
      result[operation] ||= {};
  ...
  for (var {
    0: algorithmName,
    1: operations
  } of ObjectEntries(algorithmDefs)) {
    if (_loop(algorithmName)) continue;
  }
  return {
    algorithms: result,
    nameMap
...
var {
  algorithms: kSupportedAlgorithms,
  nameMap: kAlgorithmNameMap
} = createSupportedAlgorithms(kAlgorithmDefinitions);
var simpleAlgorithmDictionaries = {
  AesCbcParams: {
    iv: 'BufferSource'
  },
  AesCtrParams: {
...
for (var {
  0: name,
  1: types
} of ObjectEntries(simpleAlgorithmDictionaries)) {
  simpleAlgorithmDictionaries[name] = {
    keys: ObjectKeys(types),
    types
  };
}
function validateMaxBufferLength(data, name) {
  var max = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : kMaxBufferLength;
  if (data.byteLength > max) {
    throw lazyDOMException(`${name} must be at most ${max} bytes`, 'OperationError');
  }
}
var webidl;
...
function normalizeAlgorithm(algorithm, op) {
  ...
  // 1.
  var registeredAlgorithms = kSupportedAlgorithms[op];
  // 2. 3.
  var initialAlg = webidl.converters.Algorithm(algorithm, kNormalizeAlgorithmOpts);
  // 4.
  var algName = initialAlg.name;

  // 5. Case-insensitive lookup via pre-built Map (O(1) instead of O(n)).
  var canonicalName = kAlgorithmNameMap[op]?.get(StringPrototypeToUpperCase(algName));
  if (canonicalName === undefined) throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
  algName = canonicalName;
  var desiredType = registeredAlgorithms[algName];
  ...
  // 6.
  var normalizedAlgorithm = webidl.converters[desiredType]({
    __proto__: algorithm,
    name: algName
  ...
  var dictMeta = simpleAlgorithmDictionaries[desiredType];
  if (dictMeta) {
    var {
      keys: dictKeys,
      types: dictTypes
    ...
    for (var i = 0; i < dictKeys.length; i++) {
      var member = dictKeys[i];
      var idlType = dictTypes[member];
      var idlValue = normalizedAlgorithm[member];
      // 3.
      if (idlType === 'BufferSource' && idlValue) {
        var isView = ArrayBufferIsView(idlValue);
        normalizedAlgorithm[member] = TypedArrayPrototypeSlice(new Uint8Array(isView ? getDataViewOrTypedArrayBuffer(idlValue) : idlValue, isView ? getDataViewOrTypedArrayByteOffset(idlValue) : 0, isView ? getDataViewOrTypedArrayByteLength(idlValue) : ArrayBufferPrototypeGetByteLength(idlValue)));
...
function getDataViewOrTypedArrayBuffer(V) {
  return isDataView(V) ? DataViewPrototypeGetBuffer(V) : TypedArrayPrototypeGetBuffer(V);
}
function getDataViewOrTypedArrayByteOffset(V) {
  return isDataView(V) ? DataViewPrototypeGetByteOffset(V) : TypedArrayPrototypeGetByteOffset(V);
}
function getDataViewOrTypedArrayByteLength(V) {
  return isDataView(V) ? DataViewPrototypeGetByteLength(V) : TypedArrayPrototypeGetByteLength(V);
}
function hasAnyNotIn(set, checks) {
  for (var s of set) if (!ArrayPrototypeIncludes(checks, s)) return true;
  return false;
}
var validateByteSource = hideStackFrames((val, name) => {
  val = toBuf(val);
  if (isAnyArrayBuffer(val) || isArrayBufferView(val)) return val;
  throw new ERR_INVALID_ARG_TYPE.HideStackFramesError(name, ['string', 'ArrayBuffer', 'TypedArray', 'DataView', 'Buffer'], val);
});
...
function jobPromise(getJob) {
  try {
    ...
    return PromiseReject(lazyDOMException('The operation failed for an operation-specific reason', {
      name: 'OperationError',
      cause: err
    }));
  }
...
function prepareWebCryptoResult(value) {
  ...
  if (isPromise(value) || ObjectPrototypeHasOwnProperty(value, 'then')) return false;
...
function resolveWebCryptoResult(resolve, value) {
  var shouldCleanupResult = prepareWebCryptoResult(value);
  try {
    resolve(value);
  } finally {
    if (shouldCleanupResult) cleanupWebCryptoResult(value);
  }
}
...
function jobPromiseThen(promise, onFulfilled, onRejected) {
  ...
  PromisePrototypeThen(promise, value => settleJobPromise(onFulfilled, resolve, reject, value, false), value => settleJobPromise(onRejected, resolve, reject, value, true));
...
function bigIntArrayToUnsignedInt(input) {
  ...
  for (var n = 0; n < input.length; ++n) {
    var n_reversed = input.length - n - 1;
 ...
 */
function getUsagesUnion(usageSet) {
  var newset = new SafeSet();
  for (var _len = arguments.length, usages = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
    usages[_key - 1] = arguments[_key];
  }
  for (var n = 0; n < usages.length; n++) {
    if (usageSet.has(usages[n])) newset.add(usages[n]);
  }
  return newset;
}

// Must be at most 31 entries.
var kCanonicalUsageOrder = ['encrypt', 'decrypt', 'sign', 'verify', 'deriveKey', 'deriveBits', 'wrapKey', 'unwrapKey', 'encapsulateKey', 'encapsulateBits', 'decapsulateKey', 'decapsulateBits'];
var kUsageMasks = {
  __proto__: null
};
var kUsageByMask = {
...
for (var n = 0; n < kCanonicalUsageOrder.length; n++) {
  var usage = kCanonicalUsageOrder[n];
  var mask = 1 << n;
...
function getUsagesMask(usageSet) {
  ...
  var mask = 0;
  for (var _usage of usageSet) {
    mask |= kUsageMasks[_usage];
  }
  return mask;
}
...
function getUsagesFromMask(mask) {
  ...
  var result = [];
  for (var _n = 0; _n < kCanonicalUsageOrder.length; _n++) {
    if (mask & 1 << _n) ArrayPrototypePush(result, kCanonicalUsageOrder[_n]);
  }
  return result;
}
...
function validateKeyOps(keyOps, usagesSet) {
  ...
  var keyOpsMask = 0;
  for (var _n2 = 0; _n2 < keyOps.length; _n2++) {
    var op = keyOps[_n2];
    var opMask = kUsageMasks[op];
    // Skipping unknown key ops
    if (opMask === undefined) continue;
    // Have we seen it already? if so, error
    if (keyOpsMask & opMask) throw lazyDOMException('Duplicate key operation', 'DataError');
    keyOpsMask |= opMask;
  ...
  if (usagesSet !== undefined) {
    var usagesMask = getUsagesMask(usagesSet);
...
function secureHeapUsed() {
  ...
  var used = Number(_secureHeapUsed());
  var total = Number(getOptionValue('--secure-heap'));
  var min = Number(getOptionValue('--secure-heap-min'));
  var utilization = used / total;
  return {
    total,
    used,
    utilization,
