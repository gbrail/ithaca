...
function dhGenerateKeys(encoding) {
  var keys = this[kHandle].generateKeys();
...
function dhComputeSecret(key, inEnc, outEnc) {
  ...
  var ret = this[kHandle].computeSecret(key);
...
ECDH.prototype.setPublicKey = deprecate(DiffieHellman.prototype.setPublicKey, 'ecdh.setPublicKey() is deprecated.', 'DEP0031');
...
ECDH.prototype.getPublicKey = function getPublicKey(encoding, format) {
  ...
  var key = this[kHandle].getPublicKey(f);
  return encode(key, encoding);
};
ECDH.convertKey = function convertKey(key, curve, inEnc, outEnc, format) {
  ...
  key = getArrayBufferOrView(key, 'key', inEnc);
  var f = getFormat(format);
  var convertedKey = _ECDHConvertKey(key, curve, f);
...
function getFormat(format) {
  if (format) {
    ...
    if (format !== 'uncompressed') throw new ERR_CRYPTO_ECDH_INVALID_FORMAT(format);
  }
...
function diffieHellman(options, callback) {
  ...
  var job = new DHBitsJob(callback ? kCryptoJobAsync : kCryptoJobSync, pubData, pubFormat, pubType, pubPassphrase, pubNamedCurve, privData, privFormat, privType, privPassphrase, privNamedCurve);
  if (!callback) {
    var {
      0: err,
      1: secret
...
function ecdhDeriveBits(algorithm, baseKey, length) {
  ...
  var keyAlgorithm = getCryptoKeyAlgorithm(key);
  var baseKeyAlgorithm = getCryptoKeyAlgorithm(baseKey);
  ...
  var bits = jobPromise(() => new DHBitsJob(kCryptoJobWebCrypto, getCryptoKeyHandle(key), undefined, undefined, undefined, undefined, getCryptoKeyHandle(baseKey), undefined, undefined, undefined, undefined));
  ...
  return jobPromiseThen(bits, bits => {
    ...
    var sliceLength = MathCeil(length / 8);
    var {
      byteLength
    } = bits;
    ...
    var slice = ArrayBufferPrototypeSlice(bits, 0, sliceLength);
    var mod = length % 8;
    if (mod === 0) return slice;

    // eslint-disable-next-line no-sparse-arrays
    masks ||= [, 0b10000000, 0b11000000, 0b11100000, 0b11110000, 0b11111000, 0b11111100, 0b11111110];
    var masked = new Uint8Array(slice);
