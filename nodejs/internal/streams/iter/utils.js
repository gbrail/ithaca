...
var kResolvedPromise = PromiseResolve();
...
function getMinCursor(consumers, fallback) {
  var minCursor = fallback;
  var minCursorConsumers = 0;
...
function concatBytes(chunks) {
  ...
  if (chunks.length === 1) {
    ...
    if (TypedArrayPrototypeGetByteOffset(chunk) === 0) {
      var buf = TypedArrayPrototypeGetBuffer(chunk);
      // SharedArrayBuffer is not available in primordials, so use
      // direct property access for its byteLength.
      var bufByteLength = isSharedArrayBuffer(buf) ? buf.byteLength : ArrayBufferPrototypeGetByteLength(buf);
  ...
  var concatenated = new Uint8Array(totalByteLength);
  var offset = 0;
  for (var _i = 0; _i < chunks.length; _i++) {
    TypedArrayPrototypeSet(concatenated, chunks[_i], offset);
    offset += TypedArrayPrototypeGetByteLength(chunks[_i]);
  }
  return concatenated;
}
