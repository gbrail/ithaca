...
var kRequestUrl = Symbol('kRequestUrl');
function isAbsoluteURLPath(path) {
  return typeof path === 'string' && (StringPrototypeStartsWith(path, 'http://') || StringPrototypeStartsWith(path, 'https://'));
}
...
var convertHeaderObject = function () {
 ...
 */
function onClientRequestCreated(_ref) {
  var {
    request
  } = _ref;
  ...
  var {
    ...
    1: host,
    2: charset
  } = convertHeaderObject(request.getHeaders());
  var url = getRequestURL(request, host);
 ...
 * @param {{ request: import('http').ClientRequest, error: any }} event
 */
function onClientResponseFinish(_ref3) {
  var {
    request,
    response
  } = _ref3;
  ...
  var {
    ...
    2: charset,
    3: mimeType
  } = convertHeaderObject(response.headers);
...
module.exports = registerDiagnosticChannels([['http.client.request.created', onClientRequestCreated], ['http.client.request.error', onClientRequestError], ['http.client.response.finish', onClientResponseFinish]]);
