...
function getHostDefinedOptionId(importModuleDynamically, hint) {
  if (importModuleDynamically === vm_dynamic_import_main_context_default || importModuleDynamically === vm_dynamic_import_default_internal) {
...
function registerImportModuleDynamically(referrer, importModuleDynamically) {
  ...
  if (importModuleDynamically === undefined || importModuleDynamically === vm_dynamic_import_main_context_default || importModuleDynamically === vm_dynamic_import_default_internal) {
    return;
  }
  var {
...
function internalCompileFunction(code, filename, lineOffset, columnOffset, cachedData, produceCachedData, parsingContext, contextExtensions, params, hostDefinedOptionId, importModuleDynamically) {
  var result = compileFunction(code, filename, lineOffset, columnOffset, cachedData, produceCachedData, parsingContext, contextExtensions, params, hostDefinedOptionId);
...
function runScriptInThisContext(script, displayErrors, breakOnFirstLine) {
  return FunctionPrototypeCall(runInContext, script, null,
  // sandbox - use current context
  -1,
  // timeout
  displayErrors,
  // displayErrors
