...
function onlookup(err, addresses) {
  ...
  var family = this.family || isIP(addresses[0]);
  this.resolve({
    address: addresses[0],
    family
  });
...
function createLookupPromise(family, hostname, all, hints, dnsOrder) {
  return new Promise((resolve, reject) => {
    if (!hostname) {
      reject(new ERR_INVALID_ARG_VALUE('hostname', hostname, 'must be a non-empty string'));
      return;
    }
    ...
    var err = getaddrinfo(req, hostname, family, hints, order);
    if (err) {
...
function createLookupServicePromise(hostname, port) {
  return new Promise((resolve, reject) => {
    ...
    req.resolve = resolve;
    req.reject = reject;
    var err = getnameinfo(req, hostname, port);
    if (err) reject(new DNSException(err, 'getnameinfo', hostname));else if (hasObserver('dns')) {
...
function lookupService(address, port) {
  if (arguments.length !== 2) throw new ERR_MISSING_ARGS('address', 'port');
...
function createResolverPromise(resolver, bindingName, hostname, ttl) {
  return new Promise((resolve, reject) => {
    ...
    req.reject = reject;
    req.ttl = ttl;
    var err = resolver._handle[bindingName](req, hostname);
    if (err) reject(new DNSException(err, bindingName, hostname));else if (hasObserver('dns')) {
...
function resolve(hostname, rrtype) {
  ...
  if (rrtype !== undefined) {
    ...
    if (typeof resolver !== 'function') throw new ERR_INVALID_ARG_VALUE('rrtype', rrtype);
  } else {
