...
function dataURLProcessor(dataURL) {
  ...
  var encodedBody = StringPrototypeSlice(input, mimeTypeLength + 1);
...
function URLSerializer(url) {
  ...
  var serialized = hashLength === 0 ? href : StringPrototypeSlice(href, 0, href.length - hashLength);
...
function collectASequenceOfCodePointsFast(char, input, position) {
  var idx = StringPrototypeIndexOf(input, char, position.position);
...
function stringPercentDecode(input) {
  ...
  var bytes = lazyEncoder().encode(input);
