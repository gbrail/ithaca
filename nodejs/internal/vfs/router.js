...
function isUnderMountPoint(normalizedPath, mountPoint) {
  ...
  var prefix = mountPoint[mountPoint.length - 1] === sep ? mountPoint : mountPoint + sep;
...
function getRelativePath(normalizedPath, mountPoint) {
  ...
  var rel = relative(mountPoint, normalizedPath);
  var segments = StringPrototypeSplit(rel, sep);
