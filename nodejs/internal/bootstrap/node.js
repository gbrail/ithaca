...
var config = internalBinding('config');
var internalTimers = require('internal/timers');
var {
  defineOperation
...
{
  var fields = process[exit_info_private_symbol];
  ...
  ObjectDefineProperty(process, 'exitCode', {
    ...
    set(code) {
      if (code !== null && code !== undefined) {
        var value = code;
        if (typeof code === 'string' && code !== '' && NumberIsNaN(value = Number(code))) {
          value = code;
        }
...
var binding = internalBinding('builtins');
var processConfig = JSONParse(binding.config, (_key, value) => {
...
var perThreadSetup = require('internal/process/per_thread');
var rawMethods = internalBinding('process_methods');
...
// Set up methods on the process object for all threads
{
  ...
  var wrapped = perThreadSetup.wrapProcessMethods(rawMethods);
  ...
  ObjectDefineProperty(process, 'finalization', {
    ...
    get() {
      ...
      var {
        createFinalization
      } = require('internal/process/finalization');
...
var credentials = internalBinding('credentials');
...
ObjectDefineProperty(process, 'allowedNodeEnvironmentFlags', {
  __proto__: null,
  get() {
    var flags = perThreadSetup.buildAllowedFlags();
...
var features = {
  ...
  get dtls() {
    return process.config.variables.node_use_dtls && hasOpenSSL && getOptionValue('--experimental-dtls');
  },
  get quic() {
    ...
    return process.config.variables.node_use_quic && !openSSLIsBoringSSL && getOptionValue('--experimental-quic') && process.config.variables.openssl_version >= 810549279; // >= 3.5.1
  }
};
...
});
{
  ...
  process.addUncaughtExceptionCaptureCallback = addUncaughtExceptionCaptureCallback;
...
var kTypeStrippingMode = process.config.variables.node_use_amaro ? null : false;
...
function setupProcessObject() {
  ...
  var origProcProto = ObjectGetPrototypeOf(process);
...
function setupBuffer() {
  ...
  var bufferBinding = internalBinding('buffer');
